...
export default class ItemWfrp4e extends Item {
   ...
   * */
	async _preCreate (data, options, user) {
		if (data._id && !this.isOwned) { options.keepId = WFRP_Utility._keepID(data._id, this); }
		...
		if (!data.img || data.img === "icons/svg/item-bag.svg") { this.updateSource({ img: "systems/wfrp4e/icons/blank.png" }); }

		if (this.isOwned) {
			// If not a character and wearable item, set worn to true
			if (this.actor.type !== "character" && this.actor.type !== "vehicle") {
				if (this.type === "armour") { this.updateSource({ "system.worn.value": true }); } else if (this.type === "weapon") { this.updateSource({ "system.equipped": true }); } else if (this.type === "trapping" && this.trappingType.value === "clothingAccessories") { this.updateSource({ "system.worn": true }); } else if (this.type === "spell") { this.updateSource({ "system.memorized.value": true }); }
			}

			if (this.type === "vehicleMod" && this.actor.type !== "vehicle") { return false; }

			if (getProperty(data, "system.location.value") && data.type !== "critical" && data.type !== "injury") { this.updateSource({ "system.location.value": "" }); }

			if (this.effects.size) {
				let immediateEffects = [];
				let conditions = [];
				this.effects.forEach(e => {
					if (e.trigger === "oneTime" && e.application === "actor") { immediateEffects.push(e); }
					if (e.isCondition) { conditions.push(e); }
				});
	...
	async _preUpdate (updateData, options, user) {
		...
		if (this.type !== "skill" || !this.isOwned || this.grouped.value !== "isSpec") { return; }
		// If no change
		...
		if (careerSkills.includes(this.name)) { careerSkills[careerSkills.indexOf(this.name)] = updateData.name; } else // if it doesn't, return
		{ return; }
	...
	prepareOwnedData () {
		try {
			...
			if (this.encumbrance && this.quantity) {
				if (this.properties?.qualities?.lightweight && this.encumbrance.value >= 1) { this.encumbrance.value -= 1; }
				if (this.properties?.flaws?.bulky) { this.encumbrance.value += 1; }

				this.encumbrance.value = (this.encumbrance.value * this.quantity.value);
				if (this.encumbrance.value % 1 != 0) { this.encumbrance.value = this.encumbrance.value.toFixed(2); }
			}
	...
	prepareOwnedContainer () {
		if (!this.countEnc.value) { this.encumbrance.value = 0; }
	}
	...
	prepareWeapon () { }
	prepareOwnedWeapon () {
		if (this.attackType === "ranged" && this.ammo && this.isOwned && this.skillToUse && this.actor.type !== "vehicle") { this._addProperties(this.ammo.properties); }

		if (this.weaponGroup.value === "flail" && !this.skillToUse && !this.flaws.value.find(i => i.name === "dangerous")) { this.flaws.value.push({ name: "dangerous" }); }
		...
		if (this.loading) {
			...
			if (this.repeater) {
				...
				if (!this.loaded.max) { this.loaded.max = 1; }
			}
		}
	}
	...
	prepareOwnedExtendedTest () {
		this.SL.pct = 0;
		if (this.SL.target > 0) { this.SL.pct = this.SL.current / this.SL.target * 100; }
		if (this.SL.pct > 100) { this.SL.pct = 100; }
		if (this.SL.pct < 0) { this.SL.pct = 0; }
	}
	...
	prepareCargo () {
		if (this.cargoType.value !== "wine" && this.cargoType.value !== "brandy") { this.quality.value = "average"; }
	}
	prepareOwnedCargo () { }
	...
	prepareOvercastingData () {
		...
		if (this.overcast?.enabled) {
			...
			if (this.overcast.initial.type === "value") {
				...
				if (this.overcast.initial.bonus) { other.initial = char.bonus; } else { other.initial = char.value; }
			...
			if (this.overcast.valuePerOvercast.type === "characteristic") {
				...
				if (this.overcast.valuePerOvercast.bonus) { other.increment = char.bonus; } else { other.increment = char.value; }
   ...
   */
	...
	_trappingExpandData () {
		...
		for (let prop of itemProperties) { data.properties.push(`<a class ='item-property'>${prop}</a>`); }
	...
	_mutationExpandData () {
		...
		if (this.modifier.value) { data.properties.push(this.modifier.value); }
		return data;
	}
	...
	_diseaseExpandData () {
		...
		if (this.permanent.value) { data.properties.push(`<b>${game.i18n.localize("Permanent")}:</b> ${this.permanent.value}`); }
		return data;
	}
	...
	_criticalExpandData () {
		...
		if (this.modifier.value) { data.properties.push(`<b>${game.i18n.localize("Modifier")}</b>: ${this.modifier.value}`); }
		return data;
	}
	...
	_spellExpandData () {
		...
		let target = this.Target;
		if (target.includes("AoE")) { target = `<a class='aoe-template' data-item-id="${this.id}" data-actor-id="${this.actor.id}"><i class="fas fa-ruler-combined"></i>${target}</a>`; }
		data.properties.push(`${game.i18n.localize("Target")}: ${target}`);
		data.properties.push(`${game.i18n.localize("Duration")}: ${this.Duration}`);
		if (this.magicMissile.value) { data.properties.push(`${game.i18n.localize("Magic Missile")}: +${this.Damage}`); } else if (this.damage.value || this.damage.dices) {
			let damage = this.Damage || "";
			if (this.damage.dice) { damage += ` + ${this.damage.dice}`; }
	...
	_prayerExpandData () {
		...
		let damage = this.Damage || "";
		if (this.damage.dice) { damage += ` + ${this.damage.dice}`; }
		if (this.damage.addSL) { damage += ` + ${game.i18n.localize("SL")}`; } // TODO: SL?
		if (this.damage.value) { data.properties.push(`${game.i18n.localize("Damage")}: ${this.DamageString}`); }
		return data;
	}
	...
	_weaponExpandData () {
		...
		if (this.weaponGroup.value) { properties.push(this.WeaponGroup); }
		if (this.range.value) { properties.push(`${game.i18n.localize("Range")}: ${this.range.value}`); }
		if (this.damage.value) {
			let damage = this.damage.value;
			if (this.damage.dice) { damage += ` + ${this.damage.dice}`; }
			properties.push(`${game.i18n.localize("Damage")}: ${damage}`);
		}
		if (this.twohanded.value) { properties.push(game.i18n.localize("ITEM.TwoHanded")); }
		if (this.reach.value) { properties.push(`${game.i18n.localize("Reach")}: ${`${game.wfrp4e.config.weaponReaches[this.reach.value]} - ${game.wfrp4e.config.reachDescription[this.reach.value]}`}`); }
		if (this.damageToItem.value) { properties.push(`${game.i18n.format("ITEM.WeaponDamaged", {damage: this.damageToItem.value})}`); }
		if (this.damageToItem.shield) { properties.push(`${game.i18n.format("ITEM.ShieldDamaged", {damage: this.damageToItem.shield})}`); }

		let itemProperties = this.OriginalQualities.concat(this.OriginalFlaws);
		for (let prop of itemProperties) { properties.push(`<a class ='item-property'>${prop}</a>`); }

		if (this.special.value) { properties.push(`${game.i18n.localize("Special")}: ${this.special.value}`); }
	...
	_cargoExpandData () {
		...
		if (this.origin.value) { data.properties.push(`<b>${game.i18n.localize("ITEM.Origin")}</b>: ${this.origin.value}`); }

		if (game.wfrp4e.config.trade.cargoTypes) { data.properties.push(`<b>${game.i18n.localize("ITEM.CargoType")}</b>: ${game.wfrp4e.config.trade.cargoTypes[this.cargoType.value]}`); }

		if (game.wfrp4e.config.trade.qualities && (this.cargoType.value === "wine" || this.cargoType.value === "brandy")) { data.properties.push(`<b>${game.i18n.localize("ITEM.CargoQuality")}</b>: ${game.wfrp4e.config.trade.qualities[this.quality.value]}`); }
   ...
   */
	async postItem (quantity) {
		...
		if (chatData.hasPrice) {
			if (!chatData.system.price.gc || isNaN(chatData.system.price.gc || 0)) { chatData.system.price.gc = 0; }
			if (!chatData.system.price.ss || isNaN(chatData.system.price.ss || 0)) { chatData.system.price.ss = 0; }
			if (!chatData.system.price.bp || isNaN(chatData.system.price.bp)) { chatData.system.price.bp = 0; }
		}
        ...
        <p>${game.i18n.localize("DIALOG.QuantityHint")}</p>
          `,
			...
			if (dialogResult.post !== "inf" && (!Number.isNumeric(dialogResult.post) || parseInt(dialogResult.post) <= 0)) { return ui.notifications.error(game.i18n.localize("CHAT.PostError")); }

			if (dialogResult.qty !== "inf" && (!Number.isNumeric(dialogResult.qty) || parseInt(dialogResult.qty) < 0)) { return ui.notifications.error(game.i18n.localize("CHAT.PostError")); }
			...
			if (dialogResult.post !== "inf") { chatData.showQuantity = true; }
   ...
   */
	...
	_trappingChatData () {
		...
		if (this.qualities.value.length) { properties.push(`<b>${game.i18n.localize("Qualities")}</b>: ${this.OriginalQualities.map(i => i = `<a class ='item-property'>${i}</a>`).join(", ")}`); }

		if (this.flaws.value.length) { properties.push(`<b>${game.i18n.localize("Flaws")}</b>: ${this.OriginalFlaws.map(i => i = `<a class ='item-property'>${i}</a>`).join(", ")}`); }
	...
	_mutationChatData () {
		...
		if (this.modifier.value) { properties.push(`<b>${game.i18n.localize("Modifier")}</b>: ${this.modifier.value}`); }
		return properties;
	}
	...
	_diseaseChatData () {
		...
		if (this.permanent.value) { properties.push(`<b>${game.i18n.localize("Permanent")}:</b> ${this.permanent.value}`); }
		return properties;
	}
	...
	_talentChatData () {
		...
		if (this.tests.value) { properties.push(`<b>${game.i18n.localize("Tests")}: </b> ${this.tests.value}`); }
		return properties;
	}
	...
	_traitChatData () {
		let properties = [];
		if (this.specification.value) { properties.push(`<b>${game.i18n.localize("Specification")}: </b> ${this.specification.value}`); }
		return properties;
	}
	...
	_injuryChatData () {
		...
		if (this.penalty.value) { properties.push(`<b>${game.i18n.localize("Penalty")}</b>: ${this.penalty.value}`); }
		return properties;
	}
	...
	_spellChatData () {
		let properties = [];
		if (game.wfrp4e.config.magicLores[this.lore.value]) { properties.push(`<b>${game.i18n.localize("Lore")}</b>: ${game.wfrp4e.config.magicLores[this.lore.value]}`); } else { properties.push(`<b>${game.i18n.localize("Lore")}</b>: ${this.lore.value}`); }
		...
		if (this.damage.value) { properties.push(`<b>${game.i18n.localize("Damage")}</b>: ${this.damage.value}`); }
	...
	_weaponChatData () {
		...
		if (this.weaponGroup.value) { properties.push(`<b>${game.i18n.localize("Group")}</b>: ${this.WeaponGroup}`); }
		if (this.range.value) { properties.push(`<b>${game.i18n.localize("Range")}</b>: ${this.range.value}`); }
		if (this.damage.value) { properties.push(`<b>${game.i18n.localize("Damage")}</b>: ${this.damage.value}`); }
		if (this.twohanded.value) { properties.push(`<b>${game.i18n.localize("ITEM.TwoHanded")}</b>`); }
		if (this.reach.value) { properties.push(`<b>${game.i18n.localize("Reach")}</b>: ${`${game.wfrp4e.config.weaponReaches[this.reach.value]} - ${game.wfrp4e.config.reachDescription[this.reach.value]}`}`); }
		if (this.damageToItem.value) { properties.push(`${game.i18n.format("ITEM.WeaponDamaged", {damage: this.damageToItem.value})}`); }
		if (this.damageToItem.shield) { properties.push(`${game.i18n.format("ITEM.ShieldDamaged", {damage: this.damageToItem.shield})}`); }
		...
		if (this.special.value) { properties.push(`<b>${game.i18n.localize("Special")}</b>: ${this.special.value}`); }
	...
	_armourChatData () {
		...
		if (this.armorType.value) { properties.push(`<b>${game.i18n.localize("ITEM.ArmourType")}</b>: ${game.wfrp4e.config.armorTypes[this.armorType.value]}`); }
		if (this.penalty.value) { properties.push(`<b>${game.i18n.localize("Penalty")}</b>: ${this.penalty.value}`); }

		for (let loc in game.wfrp4e.config.locations) {
			if (this.AP[loc]) { properties.push(`<b>${game.wfrp4e.config.locations[loc]} AP</b>: ${this.currentAP[loc]}/${this.AP[loc]}`); }
		}
	...
	_extendedTestChatData () {
		let properties = [];
		let pct = 0;
		if (this.SL.target > 0) { pct = this.SL.current / this.SL.target * 100; }
		if (pct > 100) { pct = 100; }
		if (pct < 0) { pct = 0; }
	...
	_cargoChatData () {
		...
		if (this.origin.value) { properties.push(`<b>${game.i18n.localize("ITEM.Origin")}</b>: ${this.origin.value}`); }

		if (game.wfrp4e.config.trade.cargoTypes) { properties.push(`<b>${game.i18n.localize("ITEM.CargoType")}</b>: ${game.wfrp4e.config.trade.cargoTypes[this.cargoType.value]}`); }

		if (game.wfrp4e.config.trade.qualities && (this.cargoType.value === "wine" || this.cargoType.value === "brandy")) { properties.push(`<b>${game.i18n.localize("ITEM.CargoQuality")}</b>: ${game.wfrp4e.config.trade.qualities[this.quality.value]}`); }
		return properties;
	}
   ...
   */
	computeSpellPrayerFormula (type, aoe = false, formulaOverride) {
		try {
			...
			if (Number.isNumeric(formula)) { return formula; }
			...
			if (formula != game.i18n.localize("You").toLowerCase() && formula != game.i18n.localize("Special").toLowerCase() && formula != game.i18n.localize("Instant").toLowerCase()) {
				...
				if (aoe) { formula = `AoE (${formula.capitalize()})`; }
			}
 ...
 */
   ...
   */
	computeWeaponFormula (type, mount) {
		...
		try {
			...
			for (let ch in this.actor.characteristics) {
				if (ch === "s" && mount) { actorToUse = mount; } else { actorToUse = this.actor; }
	...
	computeRangeBands () {
		let range = this.Range;
		if (!range || this.attackType === "melee") { return; }
		...
		if (game.settings.get("wfrp4e", "mooRangeBands")) {
			...
			if (!this.getFlag("wfrp4e", "optimalRange")) { game.wfrp4e.utility.log(`Warning: No Optimal Range set for ${this.name}`); }
	...
	_addProperties (properties) {
		...
		for (let q in properties.qualities) {
			...
			} else { qualities.push({ name: q, value: properties.qualities[q].value }); }
		}
		for (let f in properties.flaws) {
			...
			} else { flaws.push({ name: f, value: properties.flaws[f].value }); }
		}
	}
		...
		if (array) {
			array.forEach(p => {
				if (propertyObject[p.name]) {
					...
					if (p.value) { properties[p.name].display += ` ${Number.isNumeric(p.value) ? p.value : `(${p.value})`}`; }
				} else if (p.custom) {
					properties[p.key] = {
						key: p.key,
						display: p.display,
	...
	_addAPLayer (AP) {
		...
		for (let loc in this.currentAP) {
			if (this.currentAP[loc] > 0) {
				...
				if (this.armorType.value === "plate" || this.armorType.value === "mail") { layer.metal = true; }
	...
	_addCareerData (career) {
		...
		if (this.type === "skill") {
			if (this.advances.value >= career.level.value * 5) { this.advances.complete = true; }
		}
 ...
 */
	_addSpellDescription () {
		...
		if (description && description.includes(game.i18n.localize("SPELL.Lore"))) { return description; }

		// Use lore override if it exists
		if (this.lore.effectString) { description += `<p>\n\n <b>${game.i18n.localize("SPELL.Lore")}</b> ${this.lore.effectString}<p>`; }
		// Otherwise, use config value for lore effect
		else if (game.wfrp4e.config.loreEffectDescriptions && game.wfrp4e.config.loreEffectDescriptions[this.lore.value]) { description += `<p>\n\n <b>${game.i18n.localize("SPELL.Lore")}</b> ${game.wfrp4e.config.loreEffectDescriptions[this.lore.value]}<p>`; }
	...
	async addCondition (effect, value = 1) {
		...
		if (existing && existing.flags.wfrp4e.value == null) { return existing; } else if (existing) {
	...
	async removeCondition (effect, value = 1) {
		...
		if (existing && existing.flags.wfrp4e.value == null) { return this.deleteEmbeddedDocuments("ActiveEffect", [existing._id]); } else if (existing) {
			await existing.setFlag("wfrp4e", "value", existing.conditionValue - value);

			if (existing.flags.wfrp4e.value <= 0) { return this.deleteEmbeddedDocuments("ActiveEffect", [existing._id]); } else { return this.updateEmbeddedDocuments("ActiveEffect", [existing]); }
		}
	}
   ...
   */
	getSkillToUse (actor) {
		...
		if (this.type === "prayer") { skill = skills.find(i => i.name.toLowerCase() == game.i18n.localize("NAME.Pray").toLowerCase()); }

		if (this.type === "trait" && this.rollable.value && this.rollable.skill) { skill = skills.find(i => i.name == this.rollable.skill); }
   ...
   */
	...
	get isEquipped () {
		if (this.type === "armour" || this.type === "container") { return !!this.worn.value; } else if (this.type === "weapon") { return !!this.equipped; } else if (this.type === "trapping" && this.trappingType.value === "clothingAccessories") { return !!this.worn; }
	}
	...
	// @@@@@@@ COMPUTED GETTERS @@@@@
	get attackType () {
		if (this.type === "weapon") { return this.modeOverride?.value || game.wfrp4e.config.groupToType[this.weaponGroup.value]; } else if (this.type === "trait" && this.rollable.damage) { return this.rollable.attackType; }
	}
	...
	get cost () {
		if (this.type === "talent") { return (this.Advances + 1) * 100; } else if (this.type === "skill") {
	...
	get ammo () {
		if (this.attackType === "ranged" && this.currentAmmo?.value && this.isOwned) { return this.actor.items.get(this.currentAmmo.value); }
	}

	get ammoList () {
		if (this.ammunitionGroup.value === "throwing") { return this.actor.getItemTypes("weapon").filter(i => i.weaponGroup.value === "throwing"); } else { return this.actor.getItemTypes("ammunition").filter(a => a.ammunitionType.value == this.ammunitionGroup.value); }
	}

	get ingredient () {
		if (this.currentIng.value) { return this.actor.items.get(this.currentIng.value); }
	}
	...
	get protects () {
		...
		for (let loc in this.AP) {
			if (this.AP[loc] > 0) { protects[loc] = true; } else { protects[loc] = false; }
		}
		return protects;
	}
	...
	get properties () {
		...
		if (this.type === "weapon" && this.isOwned && !this.skillToUse && this.actor.type !== "vehicle") {
			...
			if (this.ammo) { properties.qualities = this.ammo.properties.qualities; }
		}
		...
		if (this.ammo) { properties.specialAmmo = this.ammo.properties.special; }
	...
	get skillModified () {
		if (this.modifier) {
			if (this.modifier.value > 0) { return "positive"; } else if (this.modifier.value < 0) { return "negative"; }
		}
		return "";
	}
	...
	get Duration () {
		...
		if (this.duration?.extendable) { duration += "+"; }
		return duration;
	}

	get Range () {
		if (this.type === "spell" || this.type === "prayer") { return this.computeSpellPrayerFormula("range"); } else if (this.type === "weapon") { return this.applyAmmoMods(this.computeWeaponFormula("range"), "range"); }
	}

	get Damage () {
		let damage;
		if (this.type === "spell") { damage = this.computeSpellDamage(this.damage.value, this.magicMissile.value); } else if (this.type === "prayer") { damage = this.computeSpellDamage(this.damage.value, false); } else if (this.type === "weapon") // Account for Durable, Math.max so durable doesn't go past damageToItem
		{ damage = this.applyAmmoMods(this.computeWeaponFormula("damage"), "damage") + (this.actor.flags[`${this.attackType}DamageIncrease`] || 0) - Math.max((this.damageToItem.value - (this.properties.qualities.durable?.value || 0)), 0); } else if (this.type === "trait" && this.rollable.damage) { damage = this.Specification; }
	...
	get DamageString () {
		...
		if (this.damage.dice) { string += `+ ${this.damage.dice}`; }

		if (this.ammo && this.ammo.damage.dice) { string += `+ ${this.ammo.damage.dice}`; }

		return string;
	}

	get mountDamage () {
		if (this.attackType !== "melee" || !this.actor.isMounted || !this.actor.mount) { return this.Damage; }
	...
	get Specification () {
		...
		if (this.specification.value) {
			...
			} else { specification = this.specification.value; }
		}
	...
	get characteristic () {
		if (!this.isOwned) { return this.system.characteristic; }
		let char;
		if (this.type === "skill") {
	...
	get trappingCategory () {
		if (this.type === "trapping") { return game.wfrp4e.config.trappingCategories[this.trappingType.value]; } else { return game.wfrp4e.config.trappingCategories[this.type]; }
	}
