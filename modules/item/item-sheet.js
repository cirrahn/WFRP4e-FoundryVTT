...
export default class ItemSheetWfrp4e extends ItemSheet {
   ...
   */
	...
	addConditionData (data) {
		...
		for (let condition of data.conditions) {
			...
			if (condition.flags.wfrp4e.value == null) { condition.boolean = true; }
		}
	}
   ...
   */
	...
	_onCharCheckboxClick (event) {
		...
		if (characteristicList.includes(charChanged)) { characteristicList.splice(characteristicList.findIndex(c => c == charChanged)); } else // If it isn't in the list, add it
			...
			case "earning":
				{
					...
					for (let sk in list) {
						...
						if (skillIndex == -1) { continue; } else { earningSkills.push(skillIndex); }
					}
	...
	async _onSymptomChange (event) {
		...
		let symtomNames = symptoms.map(s => {
			if (s.includes("(")) { return s.substring(0, s.indexOf("(") - 1); } else return s;
		});
	...
	_onEffectTitleClick (ev) {
		if (this.item.isOwned) { return ui.notifications.warn(game.i18n.localize("ERROR.EditEffect")); }
	...
	_onConditionClick (ev) {
		...
		if (ev.button == 0) { this.item.addCondition(condKey); } else if (ev.button == 2) { this.item.removeCondition(condKey); }
	}
	...
	_onConditionToggle (ev) {
		...
		if (game.wfrp4e.config.statusEffects.find(e => e.id == condKey).flags.wfrp4e.value == null) {
			if (this.item.hasCondition(condKey)) { this.item.removeCondition(condKey); } else { this.item.addCondition(condKey); }
			return;
		}
