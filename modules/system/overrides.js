...
export default function () {
   ...
   */
	...
	Token.prototype.incrementCondition = async function (effect, { active, overlay = false } = {}) {
		...
		if (!existing || Number.isNumeric(getProperty(existing, "flags.wfrp4e.value"))) { this.actor.addCondition(effect.id); } else if (existing) // Not numeric, toggle if existing
