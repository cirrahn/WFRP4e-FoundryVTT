...
export default class WFRP_Utility {
	static async loadTablesPath (path) {
		...
		let records;
		if (resp.error || !resp.target.includes("tables")) { throw ""; }
			...
			try {
				...
				if (records.extend && WFRP_Tables[filename] && WFRP_Tables[filename].columns) {
					...
					WFRP_Tables[filename].rows.forEach((obj, row) => {
						for (let c of records.columns) { WFRP_Tables[filename].rows[row].range[c] = records.rows[row].range[c]; }
					});
	...
	static _keepID (id, document) {
		try {
			...
			} else if (world) { collection = document.collection; }
	...
	static propertyStringToArray (propertyString, propertyObject) {
		...
		for (let property of oldProperties) {
			...
			if (newProperty) { newProperties.push(newProperty); } else { newProperties.push(property); }
		}
   ...
   */
	static async speciesCharacteristics (species, average, subspecies) {
		...
		if (subspecies && game.wfrp4e.config.subspecies[species][subspecies].characteristics) { characteristicFormulae = game.wfrp4e.config.subspecies[species][subspecies].characteristics; }
	...
	static speciesSkillsTalents (species, subspecies) {
		...
		if (subspecies && game.wfrp4e.config.subspecies[species][subspecies].skills) { skills = game.wfrp4e.config.subspecies[species][subspecies].skills; }

		if (subspecies && game.wfrp4e.config.subspecies[species][subspecies].talents) { talents = game.wfrp4e.config.subspecies[species][subspecies].talents; }
   ...
   */
	static speciesMovement (species, subspecies) {
		...
		if (subspecies && game.wfrp4e.config.subspecies[species].movement) { move = game.wfrp4e.config.subspecies[species].movement; }
		return move;
	}
   ...
   */
	static findKey (value, obj, options = {}) {
		...
		if (options.caseInsensitive) {
			for (let key in obj) {
				if (obj[key].toLowerCase() == value.toLowerCase()) { return key; }
			}
		} else {
			for (let key in obj) {
	...
	static findItemId (id, type) {
		...
		if (game.items.has(id)) { return game.items.get(id); }
   ...
   */
	static async _findBaseName (name, collection, pack) {
		...
		if (!searchResult) { searchResult = collection.find(t => t.name.split("(")[0].trim() == name.split("(")[0].trim()); }

		if (searchResult) {
			let item;
			if (pack) // If compendium pack
			{ item = await pack.getDocument(searchResult._id); } else // World Item
			{
   ...
   */
	static async findItem (itemName, itemType) {
		...
		let items;
		if (itemType) { items = game.items.contents.filter(i => i.type == itemType); } else { items = game.items.contents; }

		// Search imported items first
		for (let i of items) {
			if (i.name == itemName && i.type == itemType) { return i; }
		}
		let itemList;
		...
		for (let pack of game.wfrp4e.tags.getPacksWithTag(itemType)) {
			...
			if (searchResult) { return await pack.getDocument(searchResult._id); }
		}
	}
   ...
   */
	static _calculateAdvCost (currentAdvances, type, modifier = 0) {
		...
		if (index >= game.wfrp4e.config.xpCost[type].length) { return game.wfrp4e.config.xpCost[type][game.wfrp4e.config.xpCost[type].length - 1] + modifier; }
   ...
   */
	static async postProperty (property) {
		...
		let propertyDescr = Object.assign(duplicate(game.wfrp4e.config.qualityDescriptions), game.wfrp4e.config.flawDescriptions);
		let propertyKey;
   ...
   */
	static parsePropertyName (property) {
		...
		if (!isNaN(property[property.length - 1])) { return property.substring(0, property.length - 2).trim(); } else if (property.includes("(")) { return property.split("(")[0].trim(); } else { return property; }
	}
   ...
   */
	static chatDataSetup (content, modeOverride, isRoll = false, {forceWhisper, alias, flavor} = {}) {
		...
		if (flavor) { chatData.flavor = flavor; }
   ...
   */
	static matchClosest (object, query, options = {}) {
		...
		for (let key of keys) {
			...
			let matchCounter = 0;
			let myword;
			if (options.matchKeys) { myword = key.toLowerCase(); } else { myword = object[key].toLowerCase(); }
   ...
   */
	static getSpeaker (speaker) {
		...
		if (speaker.token) { actor = game.scenes.get(speaker.scene).tokens.get(speaker.token).actor; }
		return actor;
	}
   ...
   */
	static async allBasicSkills () {
		...
		if (!packs.length) { return ui.notifications.error(game.i18n.localize("ERROR.Found")); }
			...
			for (let i of items) {
				if (i.system.advanced.value === "bsc") {
					if (i.system.grouped.value !== "noSpec") {
						...
						if (returnSkills.filter(x => x.name.includes(skill.name)).length <= 0) { returnSkills.push(skill); }
					} else { returnSkills.push(i.toObject()); }
				}
			}
		}
   ...
   */
	static _replaceCustomLink (match, entityType, id, name) {
		...
		switch (entityType) {
			case "Roll":
				return `<a class="chat-roll" data-roll="${ids[0]}"><i class='fas fa-dice'></i> ${name || id}</a>`;
			case "Table":
			...
			case "Symptom":
				return `<a class = "symptom-tag" data-symptom="${ids[0]}"><i class='fas fa-user-injured'></i> ${name || id}</a>`;
			case "Condition":
				return `<a class = "condition-chat" data-cond="${ids[0]}"><i class='fas fa-user-injured'></i> ${name || id}</a>`;
			case "Pay":
				return `<a class = "pay-link" data-pay="${ids[0]}"><i class="fas fa-coins"></i> ${name || id}</a>`;
			case "Credit":
				return `<a class = "credit-link" data-credit="${ids[0]}"><i class="fas fa-coins"></i> ${name || id}</a>`;
			case "Corruption":
				return `<a class = "corruption-link" data-strength="${ids[0]}"><img src="systems/wfrp4e/ui/chaos.svg" height=15px width=15px style="border:none"> ${name || id}</a>`;
			case "Fear":
			...
			case "Exp":
				return `<a class = "exp-link" data-amount="${ids[0]}" data-reason="${ids[1] || ""}"><i class="fas fa-plus"></i> ${name || (ids[1] || ids[0])}</a>`;
		}
	}
   ...
   */
	static async handleTableClick (event) {
		...
		if (event.button == 0) {
			...
			chatOptions["type"] = 0;
			if (html) { ChatMessage.create(chatOptions); }
		}
	}
   ...
   */
	static handleConditionClick (event) {
		...
		if (!cond) { cond = event.target.text.trim(); }
   ...
   */
	static handleSymptomClick (event) {
		...
		if (!symptom) { symptom = event.target.text; }
   ...
   */
	static async handleRollClick (event) {
		...
		if (!roll) { roll = event.target.text.trim(); }
 ...
 */
	static handlePayClick (event) {
		...
		if (game.user.isGM) { MarketWfrp4e.generatePayCard(payString); }
	}
	...
	static handleCreditClick (event) {
		...
		if (game.user.isGM) { MarketWfrp4e.processCredit(amt, option); }
	}
	...
	static postExp (amount, reason = undefined) {
		if (isNaN(amount)) { return ui.notifications.error(game.i18n.localize("ERROR.Experience")); }
	...
	static async applyEffectToTarget (effect, targets) {
		if (!targets && !game.user.targets.size) { return ui.notifications.warn(game.i18n.localize("WARNING.Target")); }
  ...
  */
	...
	static updateGroupAdvantage ({players = undefined, enemies = undefined} = {}) {
		if (!game.user.isGM) {
			...
			if (Number.isNumeric(players)) { advantage.players = players; }
			if (Number.isNumeric(enemies)) { advantage.enemies = enemies; }
	...
	static optimalDifference (weapon, range) {
		...
		if (!weaponRange || !rangeKey) { return 1; }
	...
	static log (message, force = false, args) {
		if (CONFIG.debug.wfrp4e || force) { console.log(`%cWFRP4e` + `%c | ${message}`, "color: gold", "color: unset", args || ""); }
	}
