...
export default class SocketHandlers {
	...
	static applyOneTimeEffect (data) {
		if (game.user.id != data.payload.userId) { return; }

		let notification = "Received Apply Effect";
		if (data.payload.effect.flags?.wfrp4e?.hide !== true) { notification += ` for ${data.payload.effect.label}`; }
	...
	static changeGroupAdvantage (data) {
		if (!game.user.isGM || !game.settings.get("wfrp4e", "useGroupAdvantage")) { return; }
