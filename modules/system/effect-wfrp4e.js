...
export default class EffectWfrp4e extends ActiveEffect {
	...
	_handleDialogChoiceScript () {
		for (let mod in this.flags.wfrp4e.effectData) {
			try {
				if (mod !== "description") { this.flags.wfrp4e.effectData[mod] = eval(this.flags.wfrp4e.effectData[mod]); }
			} catch (e) {
				console.error("Error parsing dialogChoice effect");
				this.flags.wfrp4e.effectData[mod] = "";
			}
		}
		if (this.flags.wfrp4e.script) { new Function(this.flags.wfrp4e.script).bind(this)(); }
	...
	get item () {
		...
		} else if (this.parent.documentName === "Item") { return this.parent; }
	}
	...
	get sourceName () {
		...
		if (sourceName === "Unknown") {
			...
			if (sourceItem && sourceItem.type === "disease" && !game.user.isGM) { sourceName = "???"; }
		}
		return sourceName;
	}
	...
	get show () {
		if (game.user.isGM || !this.getFlag("wfrp4e", "hide")) { return true; } else { return false; }
	}
	...
	reduceItemQuantity () {
		if (this.reduceQuantity && this.item) {
			if (this.item.quantity.value > 0) { this.item.update({"system.quantity.value": this.item.quantity.value - 1}); } else { throw ui.notifications.error(game.i18n.localize("EFFECT.QuantityError")); }
		}
	}

	get displayLabel () {
		if (this.count > 1) { return `${this.label} (${this.count})`; } else return this.label;
	}
