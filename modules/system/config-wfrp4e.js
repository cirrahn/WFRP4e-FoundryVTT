...
CONFIG.TextEditor.enrichers = CONFIG.TextEditor.enrichers.concat([
	...
	},
	{
		...
		enricher: (match, options) => {
			...
			let id = match[1];
			let label = match[2];
			a.innerHTML = `<i class="fas fa-user-injured"></i>${label || id}`;
			return a;
		},
	},
	{
	...
	},
	{
		...
		enricher: (match, options) => {
			...
			let id = match[1];
			let label = match[2];
			a.innerHTML = `<i class="fas fa-coins"></i>${label || id}`;
			return a;
		},
	},
	{
	...
	},
	{
		...
		enricher: (match, options) => {
			...
			let id = match[1];
			let label = match[2];
			a.innerHTML = `<img src="systems/wfrp4e/ui/chaos.svg" height=15px width=15px style="border:none">${label || id}`;
			return a;
		},
	},
	{
	...
	},
	{
		...
		enricher: (match, options) => {
			...
			let label = match[2];
			a.innerHTML = `<i class="fas fa-plus"></i> ${label || (values[1] || values[0])}</a>`;
			return a;
		},
	},
]);
