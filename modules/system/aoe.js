...
export default class AbilityTemplate extends MeasuredTemplate {
   ...
   */
	...
	static fromString (aoeString, actorId, itemId, messageId, diameter = true) {
		if (aoeString.toLowerCase().includes(game.i18n.localize("AoE").toLowerCase())) { aoeString = aoeString.substring(aoeString.indexOf("(") + 1, aoeString.length - 1); }
   ...
   */
	...
	static updateAOETargets (templateData) {
		...
		canvas.tokens.placeables.forEach(t => {
			if ((t.x + (t.width / 2)) < maxx && (t.x + (t.width / 2)) > minx && (t.y + (t.height / 2)) < maxy && (t.y + (t.height / 2)) > miny) { newTokenTargets.push(t.id); }
		});
