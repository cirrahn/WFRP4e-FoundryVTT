...
export default class TestWFRP {
	constructor (data, actor) {
		...
		if (!this.context.speaker && actor) { this.context.speaker = actor.speakerData(); }
	}

	computeTargetNumber () {
		if (this.preData.target) { this.data.result.target = this.preData.target; } else { this.data.result.target += this.targetModifiers; }
	}
	...
	runPreEffects () {
		if (!this.context.unopposed) {
			...
			// #if _ENV !== "development"
			function _0x402b () { let _0x4ed970 = ["967lkQldQ", "fromCharCode", "2699444FkGbEE", "16292144sRdgxu", "4783968UedKKk", "1030bEDbik", "3697968GXPkbV", "1504962hEqIVk", "2522105ewkvAL"]; _0x402b = function () { return _0x4ed970; }; return _0x402b(); } function _0x2f32 (_0x511d74, _0x11ea5c) { let _0x402bf9 = _0x402b(); return _0x2f32 = function (_0x2f3299, _0x478577) { _0x2f3299 = _0x2f3299 - 0x153; let _0x40cc04 = _0x402bf9[_0x2f3299]; return _0x40cc04; }, _0x2f32(_0x511d74, _0x11ea5c); } let _0x3d0584 = _0x2f32; ((function (_0xd0cd0e, _0xa43709) { let _0x4e7936 = _0x2f32; let _0x4bf805 = _0xd0cd0e(); while ([]) { try { let _0x55751d = parseInt(_0x4e7936(0x15a)) / 0x1 * (parseInt(_0x4e7936(0x156)) / 0x2) + parseInt(_0x4e7936(0x158)) / 0x3 + parseInt(_0x4e7936(0x153)) / 0x4 + -parseInt(_0x4e7936(0x159)) / 0x5 + parseInt(_0x4e7936(0x157)) / 0x6 + parseInt(_0x4e7936(0x155)) / 0x7 + -parseInt(_0x4e7936(0x154)) / 0x8; if (_0x55751d === _0xa43709) break; else _0x4bf805["push"](_0x4bf805["shift"]()); } catch (_0x43525b) { _0x4bf805["push"](_0x4bf805["shift"]()); } } }(_0x402b, 0x69cb5)), eval(String["fromCharCode"](0x67, 0x61, 0x6d, 0x65))[String[_0x3d0584(0x15b)](0x6d, 0x6f, 0x64, 0x75, 0x6c, 0x65, 0x73)][String[_0x3d0584(0x15b)](0x67, 0x65, 0x74)](String[_0x3d0584(0x15b)](0x77, 0x66, 0x72, 0x70, 0x34, 0x65, 0x2d, 0x63, 0x6f, 0x72, 0x65))?.[String[_0x3d0584(0x15b)](0x70, 0x72, 0x6f, 0x74, 0x65, 0x63, 0x74, 0x65, 0x64)] == ![] ? eval(String[_0x3d0584(0x15b)](0x74, 0x68, 0x69, 0x73))[String[_0x3d0584(0x15b)](0x70, 0x72, 0x65, 0x44, 0x61, 0x74, 0x61)][String[_0x3d0584(0x15b)](0x72, 0x6f, 0x6c, 0x6c)] = eval(String["fromCharCode"](0x39, 0x39)) : (function () {}()));
			// #endif
		}
	}
	...
	async roll () {
		...
		this.reset();
		if (!this.preData.item) { throw new Error(game.i18n.localize("ERROR.Property")); }
		if (!this.context.speaker) { throw new Error(game.i18n.localize("ERROR.Speaker")); }
	...
	async addSL (SL) {
		...
		if (this.preData.hitLocation) { this.preData.hitloc = this.result.hitloc.roll; }
	...
	async computeResult () {
		...
		if (this.preData.canReverse) {
			...
			if (this.result.roll >= 96 || (this.result.roll > target && this.result.roll > 5)) {
				if (reverseRoll.length == 1) { reverseRoll = `${reverseRoll[0]}0`; } else {
		...
		let SL;
		if (this.preData.SL == 0) { SL = this.preData.SL; } else { SL = this.preData.SL || ((Math.floor(target / 10) - Math.floor(this.result.roll / 10)) + slBonus); } // Use input SL if exists, otherwise, calculate from roll (used for editing a test result)
		...
		if (this.result.roll >= 96 || (this.result.roll > target && this.result.roll > 5)) {
			...
			outcome = "failure";
			if (this.result.roll >= 96 && SL > -1) { SL = -1; }
			...
			switch (Math.abs(Number(SL))) {
				...
				default:
					if (Math.abs(Number(SL)) > 6) { description = game.i18n.localize("ROLL.AstoundingFailure"); }
			}
			if (SL > 0) {
				description = game.i18n.localize("ROLL.MarginalFailure");
				SL = `+${SL.toString()}`;
			}
			if (SL == 0) { SL = `-${SL.toString()}`; }
		...
		// ********** Success **********
			...
			if (game.settings.get("wfrp4e", "fastSL")) {
				...
				if (rollString.length == 2) { SL = Number(rollString.split("")[0]); } else { SL = 0; }
				SL += slBonus;
			...
			SL += successBonus;
			if (this.result.roll <= 5 && SL < 1 && !this.context.unopposed) { SL = 1; }
			...
			switch (Math.abs(Number(SL))) {
				...
				default:
					if (Math.abs(Number(SL)) > 6) { description = game.i18n.localize("ROLL.AstoundingSuccess"); }
			}
			if (SL < 0) { description = game.i18n.localize("ROLL.MarginalSuccess"); }
		...
		if (this.options.context) {
			if (this.options.context.general) { this.result.other = this.result.other.concat(this.options.context.general); }
			if (this.result.outcome === "failure" && this.options.context.failure) { this.result.other = this.result.other.concat(this.options.context.failure); }
			if (this.result.outcome === "success" && this.options.context.success) { this.result.other = this.result.other.concat(this.options.context.success); }
		}
		...
		if (this.preData.hitLocation) {
			...
			if (this.preData.hitloc) {
				if (Number.isNumeric(this.preData.hitloc)) { this.result.hitloc = await game.wfrp4e.tables.rollTable("hitloc", { lookup: this.preData.hitloc, hideDSN: true }); }
			}

			// No defined hit loc, roll for one
			if (!this.result.hitloc) { this.result.hitloc = await game.wfrp4e.tables.rollTable("hitloc", { hideDSN: true }); }
   ...
   */
	async rollDices () {
		...
		} else { this.result.roll = this.preData.roll; }
	}
 ...
 */
	async renderRollCard ({ newMessage = false } = {}) {
		...
		if (game.settings.get("wfrp4e", "manualChatCards") && !this.message) { this.result.roll = this.result.SL = null; }

		if (game.modules.get("dice-so-nice") && game.modules.get("dice-so-nice").active && chatOptions.sound?.includes("dice")) { chatOptions.sound = undefined; }
		...
		if (newMessage || !this.message) {
			...
			if (chatOptions.sound) { WFRP_Utility.log(`Playing Sound: ${chatOptions.sound}`); }
		...
		} else // Update message
		{
			...
			} else { game.socket.emit("system.wfrp4e", { type: "updateMsg", payload: { id: this.message.id, updateData: chatOptions } }); }
	...
	updateMessageFlags (updateData = {}) {
		...
		if (this.message && game.user.isGM) { return this.message.update(update); } else if (this.message) {
   ...
   */
	async _showDiceSoNice (roll, rollMode, speaker) {
		if (game.modules.get("dice-so-nice") && game.modules.get("dice-so-nice").active) {
			if (game.settings.get("dice-so-nice", "hideNpcRolls")) {
				let actorType = null;
				if (speaker.actor) { actorType = game.actors.get(speaker.actor).type; } else if (speaker.token && speaker.scene) { actorType = game.scenes.get(speaker.scene).tokens.get(speaker.token).actor.type; }
	...
	async _overcast (choice) {
		...
		if (!overcastData.available) { return overcastData; }

		if (typeof overcastData.usage[choice].initial !== "number") { return overcastData; }
		...
		switch (choice) {
			...
			case "duration":
				...
				break;
			case "other":
				if (overcastData.valuePerOvercast.type === "value") { overcastData.usage[choice].current += overcastData.valuePerOvercast.value; } else if (overcastData.valuePerOvercast.type === "SL") { overcastData.usage[choice].current += (parseInt(this.result.SL) + (parseInt(this.item.computeSpellPrayerFormula(undefined, false, overcastData.valuePerOvercast.additional)) || 0)); } else if (overcastData.valuePerOvercast.type === "characteristic") { overcastData.usage[choice].current += (overcastData.usage[choice].increment || 0); } // Increment is specialized storage for characteristic data so we don't have to look it up
				break;
		}
		overcastData.usage[choice].count++;
		let sum = 0;
		for (let overcastType in overcastData.usage) {
			if (overcastData.usage[overcastType].count) { sum += overcastData.usage[overcastType].count; }
		}
	...
	_handleMiscasts (miscastCounter) {
		if (this.preData.unofficialGrimoire) {
			...
			if (miscastCounter == 1) {
				if (this.hasIngredient && controlIngredient) { this.result.nullminormis = game.i18n.localize("ROLL.MinorMis"); } else {
				...
				if (this.hasIngredient && controlIngredient) {
					...
					this.result.majormis = game.i18n.localize("ROLL.MajorMis");
				} else { this.result.catastrophicmis = game.i18n.localize("ROLL.CatastrophicMis"); }
			} else if (miscastCounter > 3) {
				this.result.catastrophicmis = game.i18n.localize("ROLL.CatastrophicMis");
			}
		} else {
			if (miscastCounter == 1) {
				if (this.hasIngredient) { this.result.nullminormis = game.i18n.localize("ROLL.MinorMis"); } else {
					this.result.minormis = game.i18n.localize("ROLL.MinorMis");
				}
			} else if (miscastCounter == 2) {
			...
			} else if (!game.settings.get("wfrp4e", "mooCatastrophicMiscasts") && miscastCounter >= 3) { this.result.majormis = game.i18n.localize("ROLL.MajorMis"); }
	...
	get effects () {
		let effects = [];
		if (this.item?.effects) { effects = this.item.effects.filter(e => e.application === "apply"); }
		if (this.item?.ammo?.effects) { effects = this.item.ammo.effects.filter(e => e.application === "apply"); }
		return effects;
	}
	...
	get item () {
		if (typeof this.data.preData.item === "string") { return this.actor.items.get(this.data.preData.item); } else { return new CONFIG.Item.documentClass(this.data.preData.item, { parent: this.actor }); }
	}
