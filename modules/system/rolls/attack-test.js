...
export default class AttackTest extends TestWFRP {
	...
	computeProperties () {
		if (this.result.outcome === "failure") {
			...
			if (this.item.properties.flaws.unreliable) { this.result.SL--; }
			if (this.item.properties.qualities.practical) { this.result.SL++; }

			if (this.item.weaponGroup?.value === "throwing") { this.result.scatter = game.i18n.localize("Scatter"); }
		} else // if success
		{
			if (this.item.properties.qualities.blast) { this.result.other.push(`<a class='aoe-template' data-type="radius"><i class="fas fa-ruler-combined"></i>${this.item.properties.qualities.blast.value} yard Blast</a>`); }

			if (this.result.roll % 11 == 0) { this.result.critical = game.i18n.localize("Critical"); }

			// Impale weapons crit on 10s numbers
			if (this.item.properties.qualities.impale && this.result.roll % 10 == 0) { this.result.critical = game.i18n.localize("Critical"); }
	...
	calculateDamage (base) {
		...
		if (this.useMount && this.actor.mount.characteristics.s.bonus > this.actor.characteristics.s.bonus) { this.result.damage = (0, eval)(this.item.mountDamage + damageToUse); } else { this.result.damage = (0, eval)(this.item.Damage + damageToUse); }

		if (this.result.charging && !this.result.other.includes(game.i18n.localize("Charging"))) { this.result.other.push(game.i18n.localize("Charging")); }
		...
		if ((this.item.properties.flaws.tiring && this.result.charging) || !this.item.properties.flaws.tiring) {
			...
			if (this.item.properties.qualities.damaging && unitValue > Number(this.result.SL)) { base = unitValue; } // If damaging, instead use the unit value if it's higher

			if (this.useMount && this.actor.mount.characteristics.s.bonus > this.actor.characteristics.s.bonus) { this.result.damage = (0, eval)(this.item.mountDamage + damageToUse); } else { this.result.damage = (0, eval)(this.item.Damage + damageToUse); }

			// Add unit die value to damage if impact
			if (this.item.properties.qualities.impact) { this.result.damage += unitValue; }
		}
