...
export default class PrayerTest extends TestWFRP {
	...
	async computeResult () {
		...
		if (this.result.outcome === "failure") {
			...
			if (unitResult == 0) { unitResult = 10; }
			if (this.result.roll % 11 == 0 || unitResult <= currentSin) {
				if (this.result.roll % 11 == 0) { this.result.color_red = true; }
	...
	async calculateDamage () {
		...
		try {
			if (this.item.DamageString && this.result.outcome === "success") { this.result.damage = Number(this.item.Damage); }
			if (this.item.damage.addSL) { this.result.damage = Number(this.result.SL) + (this.result.damage || 0); }
