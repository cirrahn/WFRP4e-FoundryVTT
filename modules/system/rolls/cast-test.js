...
export default class CastTest extends TestWFRP {
	...
	computeTargetNumber () {
		try {
			...
			if (this.preData.skillSelected.char) { this.result.target = this.actor.characteristics[this.preData.skillSelected.key].value; } else if (this.preData.skillSelected.name == this.item.skillToUse.name) { this.result.target = this.item.skillToUse.total.value; } else if (typeof this.preData.skillSelected === "string") {
				let skill = this.actor.getItemTypes("skill").find(s => s.name == this.preData.skillSelected);
				if (skill) { this.result.target = skill.total.value; }
			} else { this.result.target = this.item.skillToUse.total.value; }
		} catch {
	...
	async calculateDamage () {
		...
		try {
			if (this.item.Damage && this.result.castOutcome === "success") { this.result.damage = Number(this.result.SL) + Number(this.item.Damage); }
	...
	postTest () {
		...
		if (this.preData.unofficialGrimoire) {
			...
			if (this.hasIngredient && this.item.ingredient.quantity.value > 0 && !this.context.edited && !this.context.reroll) { this.item.ingredient.update({ "system.quantity.value": this.item.ingredient.quantity.value - 1 }); }
		}
		...
		if (this.result.miscastModifier) {
			if (this.result.minormis) { this.result.minormis += ` (${this.result.miscastModifier})`; }
			if (this.result.majormis) { this.result.majormis += ` (${this.result.miscastModifier})`; }
			if (this.result.catastrophicmis) { this.result.catastrophicmis += ` (${this.result.miscastModifier})`; }
		}
		...
		if (this.item.cn.SL > 0) {
			if (this.result.castOutcome === "success" || !game.settings.get("wfrp4e", "mooCastAfterChannelling")) {
				...
				if (this.item.cn.SL > 0 && this.result.castOutcome === "failure") { this.result.other.push(game.i18n.localize("MOO.FailedCast")); }
			}
		}
		// @/HOUSE
	}
	...
	get effects () {
		...
		if (this.item.system.lore.effect?.application === "apply") { effects.push(this.item.system.lore.effect); }
		return effects;
	}
	...
	get characteristicKey () {
		if (this.preData.skillSelected.char) { return this.preData.skillSelected.key; } else {
			let skill = this.actor.getItemTypes("skill").find(s => s.name == this.preData.skillSelected);
			if (skill) { return skill.characteristic.key; }
		}
	}
}
