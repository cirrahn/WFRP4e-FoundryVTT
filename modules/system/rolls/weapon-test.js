...
export default class WeaponTest extends AttackTest {
	...
	computeTargetNumber () {
		...
		try {
			if (this.preData.skillSelected.char) { this.result.target = this.actor.characteristics[this.preData.skillSelected.key].value; } else if (this.preData.skillSelected.name == this.item.getSkillToUse(this.actor).name) { this.result.target = this.item.getSkillToUse(this.actor).total.value; } else if (typeof this.preData.skillSelected === "string") {
			...
			} else { this.result.target = this.item.getSkillToUse(this.actor).total.value; }
		} catch {
	...
	async roll () {
		if (this.options.offhand && this.options.offhandReverse) { this.preData.roll = this.options.offhandReverse; }
	...
	async calculateDamage () {
		...
		if (game.settings.get("wfrp4e", "mooRangedDamage")) {
			...
			if (weapon.attackType === "ranged") {
				...
				if (this.result.damage < 0) { this.result.damage = 0; }
			}
		}
		// @/HOUSE
	}
	...
	async postTest () {
		...
		if (target) {
			...
			for (let layer of AP.layers) {
				if (layer.impenetrable) { impenetrable = true; }
			}
	...
	handleDualWielder () {
		if (this.preData.dualWielding && !this.context.edited) {
			...
			if (!this.actor.hasSystemEffect("dualwielder")) { this.actor.addSystemEffect("dualwielder"); }

			if (this.result.outcome === "success") {
				let offhandWeapon = this.actor.getItemTypes("weapon").find(w => w.offhand.value);
				if (this.result.roll % 11 == 0 || this.result.roll == 100) { delete offHandData.roll; } else {
					let offhandRoll = this.result.roll.toString();
					if (offhandRoll.length == 1) { offhandRoll = `${offhandRoll[0]}0`; } else { offhandRoll = offhandRoll[1] + offhandRoll[0]; }
	...
	get vehicle () {
		if (this.options.vehicle) { return WFRP_Utility.getSpeaker(this.options.vehicle); }
	}
	...
	get item () {
		...
		if (typeof this.preData.item === "string") { return actor.items.get(this.preData.item); } else { return new CONFIG.Item.documentClass(this.preData.item, { parent: actor }); }
	}
}
