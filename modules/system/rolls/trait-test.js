...
export default class TraitTest extends AttackTest {
	...
	computeTargetNumber () {
		try {
			...
			if (this.preData.options.characteristicToUse && this.preData.options.characteristicToUse != this.item.rollable.rollCharacteristic) { this.result.target = this.actor.characteristics[this.preData.options.characteristicToUse].value; } else { this.result.target = this.actor.characteristics[this.item.rollable.rollCharacteristic].value; }

			if (this.item.skillToUse) { this.result.target += this.item.skillToUse.advances.value; }
		} catch {
	...
	get characteristicKey () {
		if (this.preData.options.characteristicToUse) { return this.preData.options.characteristicToUse; } else { return this.item.rollable.rollCharacteristic; }
	}
}
