...
export default class ChannelTest extends TestWFRP {
	...
	computeTargetNumber () {
		...
		try {
			if (this.preData.skillSelected.char) { this.result.target = this.actor.characteristics[this.preData.skillSelected.key].value; } else {
				let skill = this.actor.getItemTypes("skill").find(s => s.name == this.preData.skillSelected.name);
				if (!skill && typeof this.preData.skillSelected === "string") { skill = this.actor.getItemTypes("skill").find(s => s.name == this.preData.skillSelected); }
			...
			if (!skill) { this.result.target = this.actor.characteristics.wp.value; } else { this.result.target = skill.total.value; }
		}
	...
	postTest () {
		...
		if (this.result.outcome === "success") {
			...
			if (Number(SL) == 0 && game.settings.get("wfrp4e", "extendedTests")) { SL = 1; }
		...
		if (Number(SL) < 0 && game.settings.get("wfrp4e", "channelingNegativeSLTests")) { SL = 0; }
