...
*/
	checkPostModifiers () {
		let didModifyAttacker = false; let didModifyDefender = false;
		...
		if (didModifyAttacker || didModifyDefender) {
			...
			if (didModifyAttacker) { modifiers.message.push(`${game.i18n.format(game.i18n.localize("CHAT.TestModifiers.FinalModifiers"), { target: modifiers.attacker.target, sl: modifiers.attacker.SL, name: this.attackerTest.actor.prototypeToken.name })}`); }
			if (didModifyDefender) { modifiers.message.push(`${game.i18n.format(game.i18n.localize("CHAT.TestModifiers.FinalModifiers"), { target: modifiers.defender.target, sl: modifiers.defender.SL, name: this.defenderTest.actor.prototypeToken.name })}`); }
		}
    ...
    */
	async evaluate () {
		try {
			...
			} else if (defenderTest.context.unopposed) { defenderTest.roll(); }
            ...
            && (attackerTest.weapon.weaponGroup.value === "bow"
              ...
              || attackerTest.weapon.weaponGroup.value === "engineering")) {
				...
				let riposte;
				if (defenderTest.weapon) { riposte = defenderTest.result.riposte && !!defenderTest.weapon.properties.qualities.fast; }
			...
			if (defender) { defender.runEffects("opposedDefender", { opposedTest: this, attackerTest, defenderTest }); }
	...
	calculateOpposedDamage () {
		...
		if (this.attackerTest.actor.getItemTypes("trait").find(i => i.name == game.i18n.localize("NAME.Swarm") && i.included) || this.defenderTest.actor.getItemTypes("trait").find(i => i.name == game.i18n.localize("NAME.Swarm"))) { sizeDiff = 0; }

		if (game.settings.get("wfrp4e", "mooSizeDamage")) { sizeDiff = 0; }
		...
		let damage;
		if (this.attackerTest.useMount) { damage = item.mountDamage; } else { damage = item.Damage; }
		...
		if (this.attackerTest.trait) {
			if (sizeDiff >= 1) { addDamaging = true; }
			if (sizeDiff >= 2) { addImpact = true; }
		}
		...
		if (this.attackerTest.item.properties) {
			...
			if (this.attackerTest.result.charging || !this.attackerTest.item.properties.flaws.tiring) {
				if (hasDamaging) { addDamaging = true; }
				if (hasImpact) { addImpact = true; }
			}
