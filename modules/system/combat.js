...
export default class CombatHelpers {
	...
	static startTurnChecks (combat) {
		...
		if (turn) {
			if (turn.actor.hasSystemEffect("dualwielder")) { turn.actor.removeSystemEffect("dualwielder"); }
	...
	static endCombatChecks (combat) {
		...
		for (let script of CombatHelpers.scripts.endCombatScripts) {
			...
			if (scriptResult) { content += `${scriptResult}<br><br>`; }
		}
	...
	static checkFearTerror (combat) {
		...
		for (let turn of combat.turns) {
			try {
				...
				if (fear) { fearCounters.push({ name: turn.name, value: `@Fear[${fear.specification.value},${turn.name}]` }); }

				let terror = turn.actor.has(game.i18n.localize("CHAT.Terror"));
				if (terror) { terrorCounters.push({ name: turn.name, value: `@Terror[${terror.specification.value},${turn.name}]` }); }
			} catch (e) {
				console.log(e);
			}
		}
		...
		if (fearCounters.length || terrorCounters.length) {
			if (fearCounters.length) { msg += `<h2>${game.i18n.localize("CHAT.Fear")}</h2>${fearCounters.map(f => `<b>${f.name}</b> - ${f.value}`).join("<br>")}`; }
			if (terrorCounters.length) { msg += `<h2>${game.i18n.localize("CHAT.Terror")}</h2>${terrorCounters.map(t => `<b>${t.name}</b> - ${t.value}`).join("<br>")}`; }
		}

		msg += CombatHelpers.checkSizeFearTerror(combat);

		if (msg) { ChatMessage.create({ content: msg }); }
	}
	...
	static checkSizeFearTerror (combat) {
		...
		for (let actor in sizeMap) {
			...
			for (let otherActor in sizeMap) {
				...
				try {
					if (size > sizeMap[otherActor]) { smallerBy[size - sizeMap[otherActor]].push(otherActor); }
				} catch (e) {

				}
			}

			if (smallerBy[1].length) { msg += game.i18n.format("CHAT.CausesFear", { fear: `@Fear[${1}, ${actor}]`, actor: actor, target: smallerBy[1].join(", ")}); }

			if (smallerBy[2].length) { msg += game.i18n.format("CHAT.CausesFear", { fear: `@Terror[${2}, ${actor}]`, actor: actor, target: smallerBy[2].join(", ")}); }

			if (smallerBy[3].length) { msg += game.i18n.format("CHAT.CausesFear", { fear: `@Terror[${3}, ${actor}]`, actor: actor, target: smallerBy[3].join(", ")}); }

			if (smallerBy[4].length) { msg += game.i18n.format("CHAT.CausesFear", { fear: `@Terror[${4}, ${actor}]`, actor: actor, target: smallerBy[4].join(", ")}); }

			if (smallerBy[5].length) { msg += game.i18n.format("CHAT.CausesFear", { fear: `@Terror[${5}, ${actor}]`, actor: actor, target: smallerBy[5].join(", ")}); }

			if (smallerBy[6].length) { msg += game.i18n.format("CHAT.CausesFear", { fear: `@Terror[${6}, ${actor}]`, actor: actor, target: smallerBy[6].join(", ")}); }
	...
	static checkCorruption (combat) {
		...
		for (let turn of combat.turns) {
			...
			if (corruption) {
				...
				if (existing) { existing.counter++; } else { corruptionCounters.push({ counter: 1, type: corruption.specification.value }); }
			}
		}
	...
	static checkDiseases (combat) {
		...
		for (let turn of combat.turns) {
			...
			if (disease) {
				...
				if (existing) { existing.counter++; } else { diseaseCounters.push({ counter: 1, type: disease.specification.value }); }
			}
		}
		let content = "";
		...
		if (diseaseCounters.length) {
			...
			for (let disease of diseaseCounters) { content += `${disease.counter} <a class="item-lookup" data-type="disease" data-open="sheet">${disease.type}</a><br>`; }
	...
	static checkEndRoundConditions (combat) {
		...
		for (let turn of combat.turns) {
			...
			for (let cond of endRoundConditions) {
				if (game.wfrp4e.config.conditionScripts[cond.statusId]) {
					...
					if (Number.isNumeric(cond.flags.wfrp4e.value)) { conditionName += ` ${cond.flags.wfrp4e.value}`; }
					msgContent = `
              <h2>${conditionName}</h2>
		...
		if (removedConditions.length) { ChatMessage.create({ content: removedConditions.join("<br>") }); }
	}
	...
	static fearReminders (combat) {
		...
		if (!fearedCombatants.length) { return; }

		fearedCombatants.forEach(c => {
			let fear = c.actor.hasCondition("fear");
			chatData.content += `<b>${c.name}</b>`;
			if (fear.flags.wfrp4e.fearName) { chatData.content += ` (${fear.flags.wfrp4e.fearName})`; }
			chatData.content += "<br>";
		});
