...
export default class Migration {
	static async migrateWorld () {
		...
		for (let p of game.packs) {
			if (p.metadata.type === "Item" && p.metadata.package === "world") { await Migration.migrateCompendium(p); }
		}
		for (let p of game.packs) {
			if (p.metadata.type === "Actor" && p.metadata.package === "world") { await Migration.migrateCompendium(p); }
		}
		for (let p of game.packs) {
			if (p.metadata.type === "Scene" && p.metadata.package === "world") { await Migration.migrateCompendium(p); }
		}
 ...
 */
	static async migrateOwnedItemEffects (actor) {
		...
		for (let item of actor.items) {
			if (item.getFlag("core", "sourceId")) {
				...
				if (sourceItem) { sourceItem = sourceItem.toObject(); }
	...
	static migrateArmourData (item) {
		...
		if (item.type === "armour" && item.system.currentAP) {
			...
			for (let loc in item.system.currentAP) {
				if (item.system.currentAP[loc] == -1) { updateData["system.APdamage"][loc] = 0; } else {
   ...
   */
	static migrateItemData (item) {
		...
		if (!isEmpty(updateData)) { console.log(`Migration data for ${item.name}`, updateData); }
		return updateData;
	}
   ...
   */
	static migrateEffectData (effect) {
		...
		if (!isEmpty(updateData)) { console.log(`Migration data for ${effect.label}`, updateData); }
		return updateData;
	}
