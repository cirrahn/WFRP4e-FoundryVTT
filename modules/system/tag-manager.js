...
export default class TagManager {
	...
	findTagsFromIndex (index) {
		let tags = [];
		index.forEach(i => {
			if (!tags.includes(i.type)) { tags.push(i.type); }
		});
		return tags;
	}

	getPacksWithTag (tags) {
		if (!tags || tags.length == 0) { return Object.keys(this.tags).map(k => game.packs.get(k)); }
		...
		for (let key in this.tags) {
			if (this.tags[key].some(t => tags.includes(t))) { keys.push(key); }
		}
