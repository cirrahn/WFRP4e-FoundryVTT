...
export default class ChatWFRP {
   ...
   */
	static async chatListeners (html) {
		...
		html.on("mousedown", ".talent-drag", async ev => {
			if (ev.button == 2) { WFRP_Utility.findTalent(ev.target.text).then(talent => talent.sheet.render(true)); }
		});
		html.on("mousedown", ".skill-drag", async ev => {
			if (ev.button == 2) { WFRP_Utility.findSkill(ev.target.text).then(skill => skill.sheet.render(true)); }
		});
	...
	static _onCardEdit (ev) {
		...
		let messageId = button.parents(".message").attr("data-message-id");
		...
		{ test.preData.roll = $(message.content).find(".card-content.test-data").attr("data-roll"); } else // If not changing hitloc, use old value for hitloc
   ...
   */
	static toggleEditable (html) {
		...
		if (!elementsToToggle.length) { elementsToToggle = $(html).find(".display-toggle"); }
	...
	static _onOvercastButtonClick (event) {
		...
		if (!msg.isOwner && !msg.isAuthor) { return ui.notifications.error("CHAT.EditError"); }
	...
	static _onHaggleClicked (event) {
		...
		if (!msg.getFlag("wfrp4e", "originalPrice")) { msg.setFlag("wfrp4e", "originalPrice", payString); } else { originalPayString = msg.getFlag("wfrp4e", "originalPrice"); }
	...
	static _onCorruptButtonClicked (event) {
		...
		if (strength !== "moderate" && strength !== "minor" && strength !== "major") { return ui.notifications.error(game.i18n.localize("ErrorCorruption")); }

		let actors = canvas.tokens.controlled.map(t => t.actor);
		if (actors.length == 0) { actors = [game.user.character]; }
		if (actors.length == 0) { return ui.notifications.error(game.i18n.localize("ErrorCharAssigned")); }
	...
	static _onFearButtonClicked (event) {
		...
		if (game.user.isGM) {
			if (!targets.length) { return ui.notifications.warn(game.i18n.localize("ErrorTarget")); }
			targets.forEach(t => {
				t.actor.applyFear(value, name);
				if (canvas.scene) game.user.updateTokenTargets([]);
			});
		} else {
			if (!game.user.character) { return ui.notifications.warn(game.i18n.localize("ErrorCharAssigned")); }
	...
	static _onExpButtonClicked (event) {
		...
		if (game.user.isGM) {
			if (!game.user.targets.size) { return ui.notifications.warn(game.i18n.localize("ErrorExp")); }
			game.user.targets.forEach(t => {
				if (!alreadyAwarded.includes(t.actor.id)) {
					t.actor.awardExp(amount, reason);
					alreadyAwarded.push(t.actor.id);
				} else { ui.notifications.notify(`${t.actor.name} already received this reward.`); }
			});
			...
			if (!game.user.character) { return ui.notifications.warn(game.i18n.localize("ErrorCharAssigned")); }
			if (alreadyAwarded.includes(game.user.character.id)) { return ui.notifications.notify(`${game.user.character.name} already received this reward.`); }
	...
	static async _onConditionScriptClick (event) {
		...
		if (combatant.actor.isOwner) { conditionResult = await game.wfrp4e.config.conditionScripts[condkey](combatant.actor); } else { return ui.notifications.error(game.i18n.localize("CONDITION.ApplyError")); }

		if (game.user.isGM) { message.update(conditionResult); } else { game.socket.emit("system.wfrp4e", { type: "updateMsg", payload: { id: msgId, updateData: conditionResult } }); }
	}
	...
	static _onApplyEffectClick (event) {
		...
		if (!actor.isOwner) { return ui.notifications.error("CHAT.ApplyError"); }
      ...
      && item.target
      && item.target.value.toLowerCase() == game.i18n.localize("You").toLowerCase()) { game.wfrp4e.utility.applyEffectToTarget(effect, [{ actor }]); } else { game.wfrp4e.utility.applyEffectToTarget(effect); }
	}
	...
	static _onOpposedImgClick (event) {
		...
		if ($(event.currentTarget).hasClass("attacker")) { speaker = oppose.attacker; } else if ($(event.currentTarget).hasClass("defender")) { speaker = oppose.defender; }
