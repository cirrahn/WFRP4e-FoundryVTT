...
export default class WFRP_Tables {
   ...
   */
	static async rollTable (tableKey, options = {}, column = null) {
		...
		if (table) {
			if (table.columns) { throw new Error(game.i18n.localize("ERROR.Column")); }
			...
			if (game.dice3d && !options.hideDSN && !options.showRoll) { await game.dice3d.showForRoll(roll); }
			...
			if (modifier == 0) { displayTotal = (0, eval)(displayTotal); } // Clean up display value if modifier 0 (59 instead of 59 + 0)
			if (rollValue <= 0 && minOne) // Min one provides a lower bound of 1 on the result
			{ rollValue = 1; } else if (rollValue <= 0) {
				return {
					roll: rollValue,
				};
			}
			...
			if (rollValue > tableSize) { rollValue = tableSize; }
			...
			if (Object.keys(game.wfrp4e.config.hitLocationTables).includes(tableKey)) { result = this.formatHitloc(rollResult, rollValue); }
			...
			if (tableKey !== "menu") { return ui.notifications.error(game.i18n.format("ERROR.Table", {key: tableKey})); } else { return this.tableMenu(); }
		}
	}
   ...
   */
	static _lookup (table, value, column = null) {
		if (column && this[table].columns) {
			for (let row of this[table].rows) {
				if (WFRP_Tables._inRange(value, row.range[column])) { return duplicate(row); }
			}
		} else if (column && this[table].multi) {
			for (let row of this[table].rows) {
				if (WFRP_Tables._inRange(value, row.range[column])) { return duplicate(row[column]); }
			}
		} else {
			for (let row of this[table].rows) {
				if (WFRP_Tables._inRange(value, row.range)) { return duplicate(row); }
			}
		}
	}
	...
	static async rollToChat (table, options = {}, column = null, rollMode) {
		...
		chatOptions.type = 0;
		if (chatOptions.content) { ChatMessage.create(chatOptions); }
	...
	static findTable (key, column) {
		...
		if (!table) {
			...
			if (tables.length > 1 && column) { table = tables.find(i => i.getFlag("wfrp4e", "column") == column); }
			...
			else if (tables.length) { table = {name: tables[0].name.split("-")[0].trim(), columns: tables}; }
		}
	...
	static getHitLocTable (key) {
		...
		if (table) {
			table.results.forEach(result => {
				if (result.flags.wfrp4e.loc) { hitloc[result.flags.wfrp4e.loc] = result.text; }
			});
		}
		return hitloc;
	}
	...
	static hitLocKeyToResult (resultKey, tableKey = "hitloc") {
		...
		if (table) {
			for (let result of table.results) {
				if (result.flags.wfrp4e?.loc == resultKey) { return this.formatHitloc(result, result.range[0]); }
			}
		}
	}
   ...
   */
	static async formatChatRoll (table, options = {}, column = null) {
		...
		if (tableObject && tableObject.columns) { return this.promptColumn(table); }
		...
		try {
			...
			if (result.roll <= 0 && !options.minOne) { return game.i18n.format("TABLE.Cancel", { result: result.roll }); }
		} catch { }
		...
		if (result.object?.documentCollection && result.object?.documentId) {
			...
			if (collection) { await collection.getDocuments(); }

			if (!collection) { collection = game.collections.get(result.object.documentCollection); }
   ...
   */
	static tableMenu () {
		...
		for (let table of tables) {
			...
			else if (tableObject && !tableObject.columns) { tableMenu += `<a data-table='${key}' class='table-click'><i class="fas fa-list"></i> <code>${key}</code></a> - ${table.name}<br>`; }
		}
	...
	static promptColumn (table) {
		...
		for (let c of tableObject.columns) { prompt += `<div><a class = "table-click" data-table="${table}" data-column = "${c.getFlag("wfrp4e", "column")}"><i class="fas fa-list"></i> ${c.name}</a></div>`; }
	...
	static scatterResult ({roll, dist}) {
      ...
      + `</table>`;
		...
		if (roll == 9) { tableHtml += game.i18n.localize("CHAT.ScatterYou"); } else if (roll == 10) { tableHtml += game.i18n.localize("CHAT.ScatterThem"); } else { tableHtml += game.i18n.localize("CHAT.ScatterNote"); }
