...
export default class OpposedWFRP {
	...
	get defenderTest () {
		...
		} else { return this.defenderMessage?.getTest(); }
	}
	...
	setAttacker (message) {
		...
		if (this.message) { return this.updateMessageFlags(); }
	}
	...
	async computeOpposeResult () {
		if (!this.attackerTest || !this.defenderTest) { throw new Error(game.i18n.localize("ERROR.Opposed")); }
	...
	renderOpposedStart () {
		return new Promise(async resolve => {
			...
			if (this.target) { defender = this.target; } else if (this.defenderTest) { defender = WFRP_Utility.getToken(this.defenderTest.context.speaker) || this.defender.prototypeToken; }
	...
	updateMessageFlags () {
		...
		if (this.message && game.user.isGM) { return this.message.update(updateData); } else if (this.message) {
	...
	colorWinnerAndLoser () {
		try {
			...
			if (!game.user.isGM) { return game.socket.emit("system.wfrp4e", { type: "updateMsg", payload: { id: this.message.id, updateData: {content} } }); } else { return this.message.update({content}); }
		} catch (e) {
 ...
 */
	...
	static updateOpposedMessage (damageConfirmation, messageId) {
		...
		if (!game.user.isGM) { return game.socket.emit("system.wfrp4e", { type: "updateMsg", payload: { id: messageId, updateData: newCard } }); } else { return resultMessage.update(newCard); }
	}
}
