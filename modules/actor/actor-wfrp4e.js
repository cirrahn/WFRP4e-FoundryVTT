...
export default class ActorWfrp4e extends Actor {
   ...
   */
	async _preCreate (data, options, user) {
		if (data._id) { options.keepId = WFRP_Utility._keepID(data._id, this); }
		...
		let createData = {};
		if (!data.items?.length) { createData.items = await this._getNewActorItems(); } else { createData.items = this.items.map(i => mergeObject(i.toObject(), game.wfrp4e.migration.migrateItemData(i), {overwrite: true})); }

		if (data.effects?.length) { createData.effects = this.effects.map(i => mergeObject(i.toObject(), game.wfrp4e.migration.migrateEffectData(i), {overwrite: true})); }
		...
		if (!data.img || data.img === "icons/svg/mystery-man.svg") {
			...
			if (data.type === "vehicle") { createData.img = "systems/wfrp4e/tokens/vehicle.png"; }
		}
		...
		if (!options.skipGroupAdvantage && hasProperty(updateData, "system.status.advantage.value") && game.settings.get("wfrp4e", "useGroupAdvantage")) {
			...
			} else if (!options.fromGroupAdvantage) // Don't send groupAdvantage updates if this update is from group advantage
			{
				...
				if (game.release.generation == 10) {
					...
					if (game.user.isGM) { delete updateData.system.status; }
				}
			}
		}
	...
	handleScrollingText (data) {
		if (hasProperty(data, "system.status.wounds.value")) { this._displayScrollingChange(getProperty(data, "system.status.wounds.value") - this.status.wounds.value); }
		if (hasProperty(data, "system.status.advantage.value")) { this._displayScrollingChange(getProperty(data, "system.status.advantage.value") - this.status.advantage.value, { advantage: true }); }
	}
   ...
   */
	prepareData () {
		...
		if (this.type === "character") { this.prepareCharacter(); }
		if (this.type === "npc") { this.prepareNPC(); }
		if (this.type === "creature") { this.prepareCreature(); }
		if (this.type === "vehicle") { this.prepareVehicle(); }
			...
			try {
				if (e.origin && e.item) // If effect comes from an item
				{
					...
					if (item.type === "disease") { // If disease, don't show symptoms until disease is actually active
						if (!item.system.duration.active) { remove = true; }
						...
						if (effectApplication === "equipped") {
							if (!item.isEquipped) { remove = true; }
						} else if (effectApplication !== "actor") // Otherwise (if effect is targeted), remove it.
						{ remove = true; }
					}
				} else // If not an item effect
				{
					if (effectApplication === "apply") { remove = true; }
				}

				if (!remove) { actorEffects.set(e.id, e); }
			} catch (error) {
   ...
   */
	prepareNonVehicle () {
		...
		if (this.flags.autoCalcWalk) { this.details.move.walk = parseInt(this.details.move.value) * 2; }

		if (this.flags.autoCalcRun) { this.details.move.run = parseInt(this.details.move.value) * 4; }
		...
		if (!game.settings.get("wfrp4e", "useGroupAdvantage")) {
			...
			} else { this.status.advantage.max = 10; }
		}
		...
		if (!hasProperty(this, "flags.autoCalcSize")) { this.flags.autoCalcSize = true; }

		// Find size based on Traits/Talents
		let size;
		let trait = this.has(game.i18n.localize("NAME.Size"));
		if (trait) { size = WFRP_Utility.findKey(trait.specification.value, game.wfrp4e.config.actorSizes); }
		if (!size) // Could not find specialization
		{
			let smallTalent = this.has(game.i18n.localize("NAME.Small"), "talent");
			if (smallTalent) { size = "sml"; } else { size = "avg"; }
		}
		...
		if (this.isMounted && !game.actors) {
			...
			if (mount) {
				if (mount.status.wounds.value == 0) { this.status.mount.mounted = false; } else {
					this.details.move.value = mount.details.move.value;

					if (this.flags.autoCalcWalk) { this.details.move.walk = mount.details.move.walk; }

					if (this.flags.autoCalcRun) { this.details.move.run = mount.details.move.run; }
				}
			}
		}
	}
 ...
 */
	prepareCharacter () {
		...
		} else { this.details.status.value = ""; }

		if (currentCareer) {
			let availableCharacteristics = currentCareer.characteristics;
			for (let char in this.characteristics) {
				if (availableCharacteristics.includes(char)) { this.characteristics[char].career = true; }
			}
		}
  ...
  /* --------------------------------------------------------------------------------------------------------- */
	...
	async setupDialog ({ dialogOptions, testData, cardOptions }, type) {
		...
		if (CONFIG.Dice.rollModes) { dialogOptions.data.rollModes = CONFIG.Dice.rollModes; } else { dialogOptions.data.rollModes = CONFIG.rollModes; }

		dialogOptions.data.dialogEffects.map(e => {
			let modifiers = [];
			if (e.modifier) { modifiers.push(`${e.modifier} ${game.i18n.localize("Modifier")}`); }
			if (e.slBonus) { modifiers.push(`${e.slBonus} ${game.i18n.localize("DIALOG.SLBonus")}`); }
			if (e.successBonus) { modifiers.push(`${e.successBonus} ${game.i18n.localize("DIALOG.SuccessBonus")}`); }
			if (e.difficultyStep) { modifiers.push(`${e.difficultyStep} ${game.i18n.localize("DIALOG.DifficultyStep")}`); }
		...
		if (testData.options.context) {
			if (typeof testData.options.context.general === "string") { testData.options.context.general = [testData.options.context.general]; }
			if (typeof testData.options.context.success === "string") { testData.options.context.success = [testData.options.context.success]; }
			if (typeof testData.options.context.failure === "string") { testData.options.context.failure = [testData.options.context.failure]; }
		}
   ...
   */
	setupSkill (skill, options = {}) {
      ...
      && !options.reload) {
		...
		} else { testData.hitLocation = "none"; }
		...
		if (options.corruption) { cardOptions.rollMode = "gmroll"; }
   ...
   */
	setupWeapon (weapon, options = {}) {
		...
		if (!weapon.id) { weapon = new CONFIG.Item.documentClass(weapon, { parent: this }); }
		...
		if (weapon.attackType === "melee") { skillCharList.push({ char: true, key: "ws", name: game.i18n.localize("CHAR.WS") }); } else if (weapon.attackType === "ranged") {
			...
			if (weapon.consumesAmmo.value && weapon.ammunitionGroup.value !== "none" && weapon.ammunitionGroup.value) {
				...
				if (options.ammo) { testData.ammo = options.ammo.find(a => a.id == weapon.currentAmmo.value); }
				if (!testData.ammo) { testData.ammo = this.items.get(weapon.currentAmmo.value); }
   ...
   */
	setupCast (spell, options = {}) {
		...
		if (skill) { castSkills.push(skill); }
		...
		if (spell.damage.value) { testData.hitLocation = true; }
   ...
   */
	setupChannell (spell, options = {}) {
		...
		if (skills.length) { channellSkills = channellSkills.concat(skills); }
		...
		if (spellLore === "witchcraft") { defaultSelection = channellSkills.indexOf(channellSkills.find(x => x.name.toLowerCase().includes(game.i18n.localize("NAME.Channelling").toLowerCase()))); }
   ...
   */
	setupPrayer (prayer, options = {}) {
		...
		if (skill) { praySkills.push(skill); }
		...
		// If the spell does damage, default the hit location to checked
		if (prayer.damage.value || prayer.damage.dice || prayer.damage.addSL) { testData.hitLocation = true; }
   ...
   */
	setupTrait (trait, options = {}) {
		if (!trait.id) { trait = new CONFIG.Item.documentClass(trait, { parent: this }); }

		if (!trait.rollable.value) { return ui.notifications.notify("Non-rollable trait"); }
		...
		if (trait.rollable.rollCharacteristic === "ws" || trait.rollable.rollCharacteristic === "bs") { testData.hitLocation = "roll"; } else { testData.hitLocation = "none"; }
	...
	setupExtendedTest (item, options = {}) {
		...
		if (item.SL.target <= 0) { return ui.notifications.error(game.i18n.localize("ExtendedError1")); }
   ...
   */
	_setupCardOptions (template, title) {
		...
		if (VideoHelper.hasVideoExtension(cardOptions.flags.img)) { game.video.createThumbnail(cardOptions.flags.img, { width: 50, height: 50 }).then(img => cardOptions.flags.img = img); }
   ...
   */
	async basicTest (test, options = {}) {
		if (test.testData) { return ui.notifications.warn(game.i18n.localize("WARNING.ActorTest")); }
  ...
  /* --------------------------------------------------------------------------------------------------------- */
	...
	prepareItems () {
		...
		for (let i of this.items) {
			...
			if (i.location && i.location.value && i.type !== "critical" && i.type !== "injury") {
				inContainers.push(i);
			} else if (i.encumbrance && i.type !== "vehicleMod") { this.status.encumbrance.current += Number(i.encumbrance.value); }
		}
	...
	computeEncumbrance () {
		if (this.type !== "vehicle") {
			...
			{ game.wfrp4e.postReadyPrepare.push(this); } else {
				if (getProperty(this, "flags.actorEnc")) {
					for (let passenger of this.passengers) { this.status.encumbrance.current += passenger.enc; }
				}
			}
		}
	...
	checkWounds () {
		if (this.type !== "vehicle" && this.flags.autoCalcWounds) {
			...
			if (this.status.wounds.max != wounds) // If change detected, reassign max and current wounds
			{
				...
				} else if (this.isOwner) { this.update({ "system.status.wounds.max": wounds, "system.status.wounds.value": wounds }); }
			}
		}
	}
 ...
 */
	_calculateWounds () {
		...
		if (this.flags.autoCalcCritW) { this.status.criticalWounds.max = tb; }
   ...
   */
	applyDamage (opposedTest, damageType = game.wfrp4e.config.DAMAGE_TYPE.NORMAL) {
		if (!opposedTest.result.damage) { return `<b>Error</b>: ${game.i18n.localize("CHAT.DamageAppliedError")}`; }
		// If no damage value, don't attempt anything
		if (!opposedTest.result.damage.value) { return game.i18n.localize("CHAT.DamageAppliedErrorTiring"); }
		...
		if (applyAP) {
			...
			for (let layer of AP.layers) {
				...
				{
					if (!game.settings.get("wfrp4e", "mooPenetrating")) { AP.ignored += layer.metal ? 1 : layer.value; }
				}
				...
				if (layer.value) {
					if (layer.armourType === "plate") { soundContext.item.armourType = layer.armourType; } else if (!soundContext.item.armourType || (soundContext.item.armourType && (soundContext.item.armourType.includes("leather")) && layer.armourType === "mail")) // set to chain if there isn't an armour type set yet, or the current armor type is leather
					{ soundContext.item.armourType = layer.armourType; } else if (!soundContext.item.armourType) { soundContext.item.armourType = "leather"; }
				}
			}
			...
			if (AP.ignored) { messageElements.push(`${AP.used}/${AP.value} ${game.i18n.localize("AP")}`); } else { messageElements.push(`${AP.used} ${game.i18n.localize("AP")}`); }
			...
			} else // RAW Shields required the shield to be used
			{
				if (opposedTest.defenderTest.weapon) {
					if (opposedTest.defenderTest.weapon.properties.qualities.shield) { shieldAP = this.status.armour.shield; }
				}
			}
			...
			if (shieldAP) { messageElements.push(`${shieldAP} ${game.i18n.localize("CHAT.DamageShield")}`); }

			// Reduce damage done by AP
			totalWoundLoss -= (AP.used + shieldAP);

			// Minimum 1 wound if not undamaging
			if (!undamaging) { totalWoundLoss = totalWoundLoss <= 0 ? 1 : totalWoundLoss; } else { totalWoundLoss = totalWoundLoss <= 0 ? 0 : totalWoundLoss; }

			try {
				if (opposedTest.attackerTest.weapon.attackType === "melee") {
					if ((opposedTest.attackerTest.weapon.Qualities.concat(opposedTest.attackerTest.weapon.Flaws)).every(p => [game.i18n.localize("PROPERTY.Pummel"), game.i18n.localize("PROPERTY.Slow"), game.i18n.localize("PROPERTY.Damaging")].includes(p))) { soundContext.outcome = "warhammer"; } // special sound for warhammer :^)
					else if (AP.used) {
						soundContext.item.type = "armour";
						if (applyAP && totalWoundLoss <= 1) { soundContext.outcome = "blocked"; } else if (applyAP) { soundContext.outcome = "normal"; }
						if (impenetrable) { soundContext.outcome = "impenetrable"; }
						if (hack) { soundContext.outcome = "hack"; }
					} else {
		...
		if (newWounds <= 0) {
			...
			// @HOUSE
				...
				if (critModifier) { updateMsg += `<br><a class ="table-click critical-roll" data-modifier=${critModifier} data-table = "crit${opposedTest.result.hitloc.value}" ><i class='fas fa-list'></i> ${game.i18n.localize("Critical")} ${critModifier > 0 ? `+${critModifier}` : critModifier}</a>`; } else { updateMsg += `<br><a class ="table-click critical-roll" data-table = "crit${opposedTest.result.hitloc.value}" ><i class='fas fa-list'></i> ${game.i18n.localize("Critical")}</a>`; }
			}
			// @/HOUSE
			else if (Math.abs(newWounds) < actor.characteristics.t.bonus && !game.settings.get("wfrp4e", "uiaCrits")) { updateMsg += `<br><a class ="table-click critical-roll" data-modifier="-20" data-table = "crit${opposedTest.result.hitloc.value}" ><i class='fas fa-list'></i> ${game.i18n.localize("Critical")} (-20)</a>`; } else { updateMsg += `<br><a class ="table-click critical-roll" data-table = "crit${opposedTest.result.hitloc.value}" ><i class='fas fa-list'></i> ${game.i18n.localize("Critical")}</a>`; }
		}
		if (hack) { updateMsg += `<br>${game.i18n.localize("CHAT.DamageAP")} ${game.wfrp4e.config.locations[opposedTest.result.hitloc.value]}`; }

		if (newWounds <= 0) { newWounds = 0; } // Do not go below 0 wounds
		...
		if (daemonicTrait) {
			...
			if (isNaN(target)) { target = target.split("").filter(char => /[0-9]/.test(char)).join(""); }
   ...
   */
	async applyBasicDamage (damage, { damageType = game.wfrp4e.config.DAMAGE_TYPE.NORMAL, minimumOne = true, loc = "body", suppressMsg = false } = {}) {
		...
		if (minimumOne && modifiedDamage <= 0) { modifiedDamage = 1; } else if (modifiedDamage < 0) { modifiedDamage = 0; }
		...
		if (!suppressMsg) { return ChatMessage.create({ content: msg }); } else return msg;
	}
 ...
 */
   ...
   */
	async _advanceSpeciesSkills () {
		...
		for (let skillIndex = 0; skillIndex < skillsSelected.length; skillIndex++) {
			if (skillIndex <= 2) { await this._advanceSkill(skillList[skillsSelected[skillIndex]], 5); } else { await this._advanceSkill(skillList[skillsSelected[skillIndex]], 3); }
		}
	}
   ...
   */
	async _advanceNPC (careerData) {
		...
		for (let advChar of careerData.characteristics) {
			if (this.characteristics[advChar].advances < 5 * careerData.level.value) { updateObj[`data.characteristics.${advChar}.advances`] = 5 * careerData.level.value; }
		}

		// Advance all skills in the career
		for (let skill of careerData.skills) { await this._advanceSkill(skill, advancesNeeded); }

		// Add all talents in the career
		for (let talent of careerData.talents) { await this._advanceTalent(talent); }
   ...
   */
	useFortuneOnRoll (message, type) {
		if (this.status.fortune.value > 0) {
			...
			if (type === "reroll") { html += `${game.i18n.format("FORTUNE.UsageRerollText", { character: `<b>${this.name}</b>` })}<br>`; } else { html += `${game.i18n.format("FORTUNE.UsageAddSLText", { character: `<b>${this.name}</b>` })}<br>`; }
   ...
   */
	...
	getTalentTests () {
		...
		for (let t of talents) {
			if (!noDups.find(i => i.name == t.name)) { noDups.push(t); }
		}
		return noDups;
	}
   ...
   */
	getPrefillData (type, item, options = {}) {
		...
		try {
			...
			if (game.settings.get("wfrp4e", "testDefaultDifficulty") && (game.combat != null)) { difficulty = game.combat.started ? "challenging" : "average"; } else if (game.settings.get("wfrp4e", "testDefaultDifficulty")) { difficulty = "average"; }

			if (this.type !== "vehicle") {
				let addAdvantage = true;

				if (type === "channelling") { addAdvantage = false; }

				// @HOUSE
				if (type === "channelling" && game.settings.get("wfrp4e", "mooMagicAdvantage")) { addAdvantage = true; }

				if (type === "cast" && game.settings.get("wfrp4e", "mooMagicAdvantage")) { addAdvantage = false; }
				// @/HOUSE
				...
				if (addAdvantage) {
					...
					if (!game.settings.get("wfrp4e", "mooAdvantage")) {
						...
						if (parseInt(this.status.advantage.value) && game.settings.get("wfrp4e", "autoFillAdvantage")) { tooltip.push(`${game.i18n.localize("Advantage")} (+${(this.status.advantage.value * game.settings.get("wfrp4e", "advantageBonus"))})`); }
					}
					// @HOUSE - +1 Success Bonus SL per advantage
						...
						if (parseInt(this.status.advantage.value) && game.settings.get("wfrp4e", "autoFillAdvantage")) { tooltip.push(`${game.i18n.localize("Advantage")} (+ ${(this.status.advantage.value * 1)})`); }
					}
					// @/HOUSE
				}
				...
				if (options.corruption || options.mutate) { difficulty = "challenging"; }

				if (options.rest || options.income) { difficulty = "average"; }
			}
			...
			if (type === "trait") { difficulty = item.rollable.defaultDifficulty || difficulty; }
			...
			if (options.modify) {
				...
				if (options.modify.difficulty) { difficulty = game.wfrp4e.utility.alterDifficulty(difficulty, options.modify.difficulty); }
			}
	...
	attackPrefillData (item, options, tooltip = []) {
		...
		if (item.type === "weapon" && item.offhand.value && !item.twohanded.value && !(item.weaponGroup.value === "parry" && item.properties.qualities.defensive)) {
			...
			modifier += ambiMod;
			if (this.flags.ambi) { tooltip.push(`${game.i18n.localize("NAME.Ambi")} (+${ambiMod})`); }
		}
	...
	rangePrefillModifiers (weapon, options, tooltip = []) {
		...
		let token;
		if (this.isToken) { token = this.token; } else { token = this.getActiveTokens()[0]?.document; }

		if (!game.settings.get("wfrp4e", "rangeAutoCalculation") || !token || !game.user.targets.size == 1 || !weapon.range?.bands) { return 0; }
		...
		try {
			...
			if (attacker) {
				...
				if (sizeDiff < 0 && (item.attackType === "melee" || target.sizeNum <= 3)) {
					...
					let sizeModifier = 0;
					if (target.details.size.value === "tiny") { sizeModifier -= 30; }
					if (target.details.size.value === "ltl") { sizeModifier -= 20; }
					if (target.details.size.value === "sml") { sizeModifier -= 10; }
					if (target.details.size.value === "lrg") { sizeModifier += 20; }
					if (target.details.size.value === "enor") { sizeModifier += 40; }
					if (target.details.size.value === "mnst") { sizeModifier += 60; }
			...
			if (this.isMounted && item.attackType === "melee" && target) {
				...
				if (target.isMounted) { mountSizeDiff = this.mount.sizeNum - target.sizeNum; }
			...
			// Attacking a creature on a larger mount
				...
				if (this.isMounted) { mountSizeDiff = target.sizeNum - this.mount.sizeNum; }
				if (mountSizeDiff >= 1) {
   ...
   */
	armourPrefillModifiers (item, type, options, tooltip = []) {
		...
		for (let a of this.getItemTypes("armour").filter(i => i.isEquipped)) {
			...
			// Skip practical
			if (a.properties.qualities.practical) { continue; }

			if (a.armorType.value === "mail") { wearingMail = true; }
			if (a.armorType.value === "plate") { wearingPlate = true; }
		}

		// Apply armor type penalties at the end
		if (wearingMail || wearingPlate) {
			let stealthPenaltyValue = 0;
			if (wearingMail) { stealthPenaltyValue += -10; }
			if (wearingPlate) { stealthPenaltyValue += -10; }
	...
	runEffects (trigger, args, options = {}) {
		...
		if (trigger === "oneTime") {
			...
			if (effects.length) { this.deleteEmbeddedDocuments("ActiveEffect", effects.map(e => e.id)); }
		}
		...
		effects.forEach(e => {
			...
			if (trigger === "targetPrefillDialog" || trigger === "prefillDialog") {
				...
				if (e.tooltip != e.label) { appliedEffects.push(e); }
			} else {
   ...
   */
	...
	async decrementInjury (injury) {
		if (isNaN(injury.system.duration.value)) { return ui.notifications.notify(game.i18n.format("CHAT.InjuryError", { injury: injury.name })); }

		injury = duplicate(injury);
		injury.system.duration.value--;

		if (injury.system.duration.value < 0) { injury.system.duration.value = 0; }
	...
	async decrementDisease (disease) {
		...
		if (!d.system.duration.active) {
			...
			if (Number.isNumeric(d.system.duration.value)) {
				...
				if (d.system.duration.value == 0) { this.finishDisease(d); }
			} else {
	...
	_applyStatusModifier ({ standing, tier }) {
		...
		if (modifier < 0) { this.details.status.modified = "negative"; } else if (modifier > 0) { this.details.status.modified = "positive"; }
		...
		if (standing <= 0 && tier !== "b") {
			standing = 5 + standing;
			if (tier === "g") { tier = "s"; } else if (tier === "s") { tier = "b"; }
		...
		// If rock bottom
			...
			standing = standing - 5;
			if (tier === "s") { tier = "g"; } else if (tier === "b") { tier = "s"; }
		...
		if (result.outcome === "success") {
			...
			switch (tier) {
				...
				case "s":
					...
					break;
				case "g":
					if (moneyEarned > 1) { result.incomeResult += ` ${game.i18n.localize("NAME.GC").toLowerCase()}.`; } else { result.incomeResult += ` ${game.i18n.localize("NAME.GCPlural").toLowerCase()}.`; }
					break;
			}
	...
	async handleCorruptionResult (test) {
		...
		switch (strength) {
			...
			case "moderate":
				if (failed) { corruption += 2; } else if (test.result.SL < 2) { corruption += 1; }
				break;

			case "major":
				if (failed) { corruption += 3; } else if (test.result.SL < 2) { corruption += 2; } else if (test.result.SL < 4) { corruption += 1; }
				break;
		}
		...
		if (test.context.reroll || test.context.fortuneUsedAddSL) {
			...
			switch (strength) {
				case "minor":
					if (previousFailed) { corruption--; }
					break;

				case "moderate":
					if (previousFailed) { corruption -= 2; } else if (test.context.previousResult.SL < 2) { corruption -= 1; }
					break;

				case "major":
					if (previousFailed) { corruption -= 3; } else if (test.context.previousResult.SL < 2) { corruption -= 2; } else if (test.context.previousResult.SL < 4) { corruption -= 1; }
					break;
			}
		}
		...
		if (!test.context.reroll && !test.context.fortuneUsedAddSL) { ChatMessage.create(WFRP_Utility.chatDataSetup(game.i18n.format("CHAT.CorruptionFail", { name: this.name, number: corruption }), "gmroll", false)); } else { ChatMessage.create(WFRP_Utility.chatDataSetup(game.i18n.format("CHAT.CorruptionReroll", { name: this.name, number: corruption }), "gmroll", false)); }

		await this.update({ "system.status.corruption.value": newCorruption });
		if (corruption > 0) { this.checkCorruption(); }
	}
	...
	async handleMutationResult (test) {
      ...
      <p>${tableText}</p>`,
		...
		} else { ChatMessage.create(WFRP_Utility.chatDataSetup(game.i18n.localize("CHAT.MutateSuccess"), "gmroll", false)); }
	}
	...
	async handleExtendedTest (test) {
		...
		if (game.settings.get("wfrp4e", "extendedTests") && test.result.SL == 0) { test.result.SL = test.result.roll <= test.result.target ? 1 : -1; }

		if (item.system.failingDecreases.value) {
			item.system.SL.current += Number(test.result.SL);
			if (!item.system.negativePossible.value && item.system.SL.current < 0) { item.system.SL.current = 0; }
		} else if (test.result.SL > 0) { item.system.SL.current += Number(test.result.SL); }
		...
		if (item.system.SL.current >= item.system.SL.target) {
			if (getProperty(item, "flags.wfrp4e.reloading")) {
				let actor;
				if (getProperty(item, "flags.wfrp4e.vehicle")) { actor = WFRP_Utility.getSpeaker(getProperty(item, "flags.wfrp4e.vehicle")); }
			...
			if (item.system.completion.value === "reset") { item.system.SL.current = 0; } else if (item.system.completion.value === "remove") {
		...
		if (item) { this.updateEmbeddedDocuments("Item", [item]); }
	}

	checkReloadExtendedTest (weapon) {
		if (!weapon.loading) { return; }
		...
		if (weapon.loaded.amt > 0) {
			...
			if (weapon.skillToUse) { reloadExtendedTest.system.test.value = weapon.skillToUse.name; } else { reloadExtendedTest.system.test.value = game.i18n.localize("CHAR.BS"); }
			...
			if (reloadingTest) { reloadingTest.delete(); }
	...
	setAdvantage (val) {
		...
		if (game.settings.get("wfrp4e", "capAdvantageIB")) { advantage.max = this.characteristics.i.bonus; } else { advantage.max = 10; }
	...
	showCharging (item) {
		if (item.attackType === "melee") { return true; }
	}
	...
	get mount () {
		if (this.status.mount.isToken) {
			...
			if (canvas.scene.id != scene?.id) { return ui.notifications.error(game.i18n.localize("ErrorTokenMount")); }
	...
	async addCondition (effect, value = 1) {
		if (typeof (effect) === "string") { effect = duplicate(game.wfrp4e.config.statusEffects.find(e => e.id == effect)); }
		...
		if (existing && !existing.isNumberedCondition) { return existing; } else if (existing) {
			existing._displayScrollingStatus(true);
			return existing.setFlag("wfrp4e", "value", existing.conditionValue + value);
		} else if (!existing) {
			if (game.combat && (effect.id === "blinded" || effect.id === "deafened")) { effect.flags.wfrp4e.roundReceived = game.combat.round; }
			effect.label = game.i18n.localize(effect.label);

			if (Number.isNumeric(effect.flags.wfrp4e.value)) { effect.flags.wfrp4e.value = value; }

			effect["flags.core.statusId"] = effect.id;
			effect["statuses"] = [effect.id]; // V11 thing, should be discarded for V10
			if (effect.id === "dead") { effect["flags.core.overlay"] = true; }
			if (effect.id === "unconscious") { await this.addCondition("prone"); }
	...
	async removeCondition (effect, value = 1) {
		...
		if (existing && !existing.isNumberedCondition) {
			if (effect.id === "unconscious") { await this.addCondition("fatigued"); }
			...
			if (existing.conditionValue <= 0) { return existing.delete(); }
		}
	}
	...
	applyFear (value, name = undefined) {
		...
		if (name) { fear.effects[0].flags.wfrp4e.fearName = name; }
	...
	_addToExpLog (amount, reason, newSpent, newTotal) {
		if (!newSpent) { newSpent = this.details.experience.spent; }
		if (!newTotal) { newTotal = this.details.experience.total; }
	...
	populateEffect (effectId, item, test) {
		...
		if (!effect && item?.ammo) { effect = item.ammo.effects.get(effectId)?.toObject(); }
		if (!effect) { return ui.notifications.error(game.i18n.localize("ERROR.EffectNotFound")); }
		...
		if (duration && item.duration.value.toLowerCase().includes(game.i18n.localize("Minutes"))) { effect.duration.seconds = duration * 60; } else if (duration && item.duration.value.toLowerCase().includes(game.i18n.localize("Hours"))) { effect.duration.seconds = duration * 60 * 60; } else if (duration && item.duration.value.toLowerCase().includes(game.i18n.localize("Rounds"))) { effect.duration.rounds = duration; }
	...
	removeSystemEffect (key) {
		...
		if (effect) { this.deleteEmbeddedDocuments("ActiveEffect", [effect.id]); }
	}
   ...
   */
	displayStatus (round = undefined, nameOverride) {
		if (round) { round = game.i18n.format("CondRound", { round: round }); }
   ...
   */
	async _getNewActorItems () {
      ...
      || [];
		...
		if (this.type === "character") { return basicSkills.concat(moneyItems); }
   ...
   */
	...
	get Species () {
		...
		if (this.details.species.subspecies && game.wfrp4e.config.subspecies[this.details.species.value] && game.wfrp4e.config.subspecies[this.details.species.value][this.details.species.subspecies]) { species += ` (${game.wfrp4e.config.subspecies[this.details.species.value][this.details.species.subspecies].name})`; } else if (this.details.species.subspecies) { species += ` (${this.details.species.subspecies})`; }
	...
	get equipPointsUsed () {
		return this.getItemTypes("weapon").reduce((prev, current) => {
			if (current.isEquipped) { prev += current.twohanded.value ? 2 : 1; }
			return prev;
		}, 0);
	}
	...
	get defensive () {
		...
		return this.getItemTypes("weapon").reduce((prev, current) => {
			if (current.isEquipped) { prev += current.properties.qualities.defensive ? 1 : 0; }
			return prev;
		}, 0) + this.getItemTypes("trait").reduce((prev, current) => {
			if (current.included) { prev += current.properties?.qualities?.defensive ? 1 : 0; }
			return prev;
		}, 0);
	}
	...
	get attacker () {
		try {
			if (this.flags.oppose) {
				...
				} else { this.update({ "flags.-=oppose": null }); }
			}
		} catch (e) {
	...
	get advantageGroup () {
		if (this.hasPlayerOwner) { return "players"; } else if (this.token) { return this.token.disposition == CONST.TOKEN_DISPOSITIONS.FRIENDLY ? "players" : "enemies"; } else { return this.prototypeToken.disposition == CONST.TOKEN_DISPOSITIONS.FRIENDLY ? "players" : "enemies"; }
	}
