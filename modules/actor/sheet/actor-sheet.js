...
export default class ActorSheetWfrp4e extends ActorSheet {
   ...
   */
	...
	_refocus (html) {
		try {
			let element;
			if (this.saveFocus) { element = html.find(`input[${this.saveFocus}]`)[0]; }
   ...
   */
	...
	_sortItemLists (items) {
		for (let prop in items) {
			if (Array.isArray(items[prop])) { items[prop] = items[prop].sort((a, b) => (a.sort || 0) - (b.sort || 0)); } else if (typeof items === "object") { this._sortItemLists(items[prop]); }
		}
	}
	_consolidateEffects (effects) {
		...
		for (let effect of effects) {
			...
			if (!existing) { consolidated.push(effect); }
		}
	...
	_consolidateTalents () {
		...
		for (let talent of talents) {
			...
			if (!existing) { consolidated.push(talent); }
		}
		return consolidated;
	}
		...
		for (let loc in AP) {
			...
			if (table) {
				try {
					...
					if (result) { AP[loc].label = game.i18n.localize(result.text); } else { AP[loc].show = false; }
				} catch (e) {
	...
	addMountData (data) {
		try {
			if (!this.actor.mount) { return; }

			data.mount = this.actor.mount.data;
			if (data.mount.system.status.wounds.value == 0) { this.actor.status.mount.mounted = false; }
			if (data.actor.status.mount.isToken) { data.mount.sceneName = game.scenes.get(data.actor.system.status.mount.tokenData.scene).name; }
		} catch (e) {
   ...
   */
	modifyWounds (value) {
		...
		{ return this.actor.modifyWounds(parseInt(value)); } else // Absolute
 ...
 */
   ...
   */
	activateListeners (html) {
		...
		html.on("drop", ".mount-drop", async ev => {
			...
			if (game.wfrp4e.config.actorSizeNums[mount.details.size.value] < game.wfrp4e.config.actorSizeNums[this.actor.details.size.value]) { return ui.notifications.error(game.i18n.localize("MountError")); }
			...
			if (this.actor.prototypeToken.actorLink && !mount.prototypeToken.actorLink) { ui.notifications.warn(game.i18n.localize("WarnUnlinkedMount")); }
	...
	_onTraitRoll (ev) {
		ev.preventDefault();
		if (ev.button == 2) { return this._onItemSummary(ev); }
	...
	_saveFocus (ev) {
		if (ev.target.attributes["data-item-id"]) { this.saveFocus = `data-item-id="${ev.target.attributes["data-item-id"].value}`; }

		if (ev.target.attributes["data-char"]) { this.saveFocus = `data-char="${ev.target.attributes["data-char"].value}`; }
	}
	...
	_onExtendedSLClick (ev) {
		...
		if (SL < 0 && !item.negativePossible.value) { SL = 0; }
	...
	_onWeaponDamageClick (ev) {
		...
		} else if (ev.button == 0) { itemData.system.damageToItem.value = Math.max(minDamage, itemData.system.damageToItem.value - 1); }
	...
	_onArmourTotalClick (ev) {
		...
		for (let armourTrait of armourTraits) {
			...
			if (armourTrait) {
				...
				if (usedTrait) { return this.actor.updateEmbeddedDocuments("Item", [armourTrait]); }
			}
		}
	...
	async _onMemorizedClick (ev) {
		...
		if (!spell.memorized.value) { WFRP_Audio.PlayContextAudio({ item: spell, action: "memorize" }); } else { WFRP_Audio.PlayContextAudio({ item: spell, action: "unmemorize" }); }
	...
	_onSpellSLClick (ev) {
		...
		switch (ev.button) {
			case 0: SL++;
				if (SL > (spell.memorized.value ? spell.cn.value : spell.cn.value * 2)) { SL = (spell.memorized.value ? spell.cn.value : spell.cn.value * 2); }
				break;
			case 2:
				SL--;
				if (SL < 0) { SL = 0; }
	...
	async _onInjuryDurationClick (ev) {
		...
		if (!isNaN(injury.system.duration.value)) {
			if (ev.button == 0) { return this.actor.decrementInjury(injury); } else injury.system.duration.value++;
	...
	_onLoadedClick (ev) {
		...
		if (item.repeater) {
			...
			if (ev.button == 2 && itemObject.system.loaded.amt <= 0) { return; }
			...
			if (itemObject.system.loaded.value) { itemObject.system.loaded.amt = itemObject.system.loaded.max || 1; } else itemObject.system.loaded.amt = 0;
		}
				...
				buttons: {
					split: {
						...
						callback: (dlg) => {
							...
							if (Number.isNumeric(amt)) { return this.splitItem(this._getItemId(ev), amt); }
						},
					},
				},
				default: "split",
	...
	_onConditionValueClicked (ev) {
		...
		if (ev.button == 0) { this.actor.addCondition(condKey); } else if (ev.button == 2) { this.actor.removeCondition(condKey); }
	}
	_onConditionToggle (ev) {
		let condKey = $(ev.currentTarget).parents(".sheet-condition").attr("data-cond-id");
		if (game.wfrp4e.config.statusEffects.find(e => e.id == condKey).flags.wfrp4e.value == null) {
			if (this.actor.hasCondition(condKey)) { this.actor.removeCondition(condKey); } else this.actor.addCondition(condKey);
			return;
		}
		if (ev.button == 0) { this.actor.addCondition(condKey); } else if (ev.button == 2) { this.actor.removeCondition(condKey); }
	}
	async _onSpeciesEdit (ev) {
		...
		let subspecies;
		if (split.length > 1) { subspecies = split[1].replace(")", "").trim(); }
				...
				case game.i18n.localize("RANDOMIZER.C"): let creatureMethod = false;
					...
					if (!creatureMethod) {
						...
						for (let char in characteristics) {
							...
							if (characteristics[char].initial < 0) { characteristics[char].initial = 0; }
						}
						await this.actor.update({ "system.characteristics": characteristics });
					}
					return;
	...
	async _onConditionClicked (ev) {
		...
		let elementToAddTo = $(ev.currentTarget).parents(".condition-list");
		let condkey = li.attr("data-cond-id"); let expandData = await TextEditor.enrichHTML(`<h2>${game.wfrp4e.config.conditions[condkey]}</h2>${game.wfrp4e.config.conditionDescriptions[condkey]}`, {async: true});
   ...
   */
	...
	async _onDropLookupItem (dragData) {
		...
		if (item) { this.actor.createEmbeddedDocuments("Item", [item.toObject()]); }
	}
	...
	_onDropMoney (dragData) {
		...
		let halfS = false; let halfG = false;
		if (type === "b") { amt = Math.round(moneyString.slice(0, -1)); } else if (type === "s") {
			if (moneyString.slice(0, -1).includes(".")) { halfS = true; }
			amt = Math.floor(moneyString.slice(0, -1));
		} else if (type === "g") {
			if (moneyString.slice(0, -1).includes(".")) { halfG = true; }
			amt = Math.floor(moneyString.slice(0, -1));
		}
		...
		if (!amt && !halfG && !halfS) { money.forEach(m => m.system.quantity.value = 0); } else // Otherwise, add amount to designated type
		{ moneyItem.system.quantity.value += amt; }

		// add halves
		if (halfS) { money.find(i => i.name === game.i18n.localize("NAME.BP")).system.quantity.value += 6; }
		if (halfG) { money.find(i => i.name === game.i18n.localize("NAME.SS")).system.quantity.value += 10; }
	...
	_dropdownListeners (html) {
		...
		html.on("click", ".apply-effect", async ev => {
			...
			if (effect.flags.wfrp4e?.reduceQuantity) {
				if (item.quantity.value > 0) { item.update({"system.quantity.value": item.quantity.value - 1}); } else { throw ui.notifications.error(game.i18n.localize("EFFECT.QuantityError")); }
			}

			if ((item.range && item.range.value.toLowerCase() == game.i18n.localize("You").toLowerCase()) && (item.target && item.target.value.toLowerCase() == game.i18n.localize("You").toLowerCase())) { game.wfrp4e.utility.applyEffectToTarget(effect, [{ actor: this.actor }]); } // Apply to caster (self)
			else { game.wfrp4e.utility.applyEffectToTarget(effect); }
		});
   ...
   */
	async _expandProperty (ev) {
		...
		let li = $(ev.currentTarget).parents(".item");
		let property = ev.target.text; // Proprety clicked on
		let properties = mergeObject(WFRP_Utility.qualityList(), WFRP_Utility.flawList()); // Property names
		let propertyDescr = Object.assign(duplicate(game.wfrp4e.config.qualityDescriptions), game.wfrp4e.config.flawDescriptions); // Property descriptions
		...
		if (propertyDescription.includes("(Rating)")) { propertyDescription = propertyDescription.replaceAll("(Rating)", property.split(" ")[1]); }
	...
	_onSortClick (ev) {
		...
		for (let i = 1; i < items.length; i++) { items[i].sort = items[i - 1].sort + 10000; }
   ...
   */
	...
	async splitItem (itemId, amount) {
		...
		if (amount >= item.system.quantity.value) { return ui.notifications.notify(game.i18n.localize("Invalid Quantity")); }
