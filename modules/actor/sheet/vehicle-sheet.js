   ...
   */
	...
	_addEncumbranceData (sheetData) {
		...
		if (sheetData.system.status.encumbrance.pct + sheetData.system.status.encumbrance.carryPct > 100) {
			...
			if (sheetData.system.status.encumbrance.pct + sheetData.system.status.encumbrance.carryPct == 100 && sheetData.system.status.encumbrance.carryPct) { sheetData.system.status.encumbrance.carryPct -= 1; }
		}
					...
					buttons: {
						select: {
							...
							callback: (dlg) => {
								...
								if (actorId) { resolve(game.actors.get(actorId)); }
								reject();
							},
						},
					},
   ...
   */
	...
	async _onRoleSkillClick (ev) {
		...
		if (ev.button == 0) {
			...
			if (!actor) { return ui.notifications.error(game.i18n.localize("VEHICLE.NoActor")); }
			if (!actor.isOwner) { return ui.notifications.error(game.i18n.localize("VEHICLE.TestNotPermitted")); }
			...
			if (!skill) {
				...
				if (!char) { return ui.notifications.error(game.i18n.localize("VEHICLE.TestNotFound")); }

				if (testLabel) { title = `${testLabel} - ${test}`; }
				...
				if (handling) { prefill.slBonus -= penalty; }
	...
	_onRoleNameClick (ev) {
		...
		if (!actor) { return ui.notifications.error(game.i18n.localize("VEHICLE.NoActor")); } else { actor.sheet.render(true); }
	}
	...
	async _onVehicleWeaponClick (ev) {
		...
		if (!game.user.isGM && game.user.character) {
			...
			if (!actor.isOwner) { return ui.notifications.error(game.i18n.localize("VEHICLE.CantUseActor")); }
