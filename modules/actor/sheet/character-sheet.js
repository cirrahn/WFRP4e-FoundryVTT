   ...
   */
		...
		for (let career of sheetData.actor.getItemTypes("career")) {
			if (career.current.value) {
				...
				for (let sk of career.skills) {
					...
					if (trainedSkill) { trainedSkill._addCareerData(career); } else { sheetData.career.untrainedSkills.push(sk); }
				}
				...
				for (let talent of career.talents) {
					...
					if (trainedTalent) { trainedTalent._addCareerData(career); } else { sheetData.career.untrainedTalents.push(talent); }
				}
			}
		}
	...
	_condenseXPLog (sheetData) {
		...
		for (let log of condensed) {
			if (log.counter && log.counter > 1) { log.reason += ` (${log.counter})`; }
		}
	...
	async _onAdvancementClick (ev) {
		...
		if (type === "skill") {
			...
			if (ev.button == 0) {
				...
				if (item.advances.value == 0) { return; }
		...
		// Characteristics
			...
			if (ev.button == 0) {
				...
				if (currentChar.advances == 0) { return; }
