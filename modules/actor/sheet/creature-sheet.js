...
export default class ActorSheetWfrp4eCreature extends ActorSheetWfrp4e {
   ...
   */
	_delayedDropdown (event) {
		// count clicks
		if (this.clicks) { this.clicks++; } else { this.clicks = 1; }
   ...
   */
	async _onCreatureItemSummary (event) {
		...
		let item = this.actor.items.get($(event.currentTarget).attr("data-item-id"));
   ...
   */
	activateListeners (html) {
		...
		html.find(".content").hover(event => {
			if (!this.dialogOpen) { $(event.currentTarget).focus(); }
		});
	...
	_onContentClick (ev) {
		if (ev.keyCode == 46) {
			...
			if (itemId) { return this.actor.deleteEmbeddedDocuments("Item", [itemId]); }
		}
	}
	...
	_onCreatureSkillClick (event) {
		...
		if (event.shiftKey || event.ctrlKey) {
			if (event.shiftKey) { advAmt = 10; } else if (event.ctrlKey) { advAmt = 1; }
		}
