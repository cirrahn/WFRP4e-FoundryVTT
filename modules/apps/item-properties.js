...
export default class ItemProperties extends FormApplication {
	...
	async _updateObject (event, formData) {
		...
		for (let prop in formData) {
			if (prop === "custom-quality") { qualities = qualities.concat(this.parseCustomProperty(formData[prop])); } else if (prop === "custom-flaw") { flaws = flaws.concat(this.parseCustomProperty(formData[prop])); }
			...
			if (formData[prop] && !prop.includes("-value")) {
				...
				if (this.qualities[prop]) { qualities.push(property); } else if (this.flaws[prop]) { flaws.push(property); }
			}
		}
