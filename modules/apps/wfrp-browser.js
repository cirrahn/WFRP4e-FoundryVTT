...
export default class BrowserWfrp4e extends Application {
   ...
   */
	addItems (itemList) {
		for (let item of itemList) {
			if (item.type === "career") {
				if (!this.careerGroups.includes(item.system.careergroup.value)) { this.careerGroups.push(item.system.careergroup.value); }
				if (!this.careerClasses.includes(item.system.class.value)) { this.careerClasses.push(item.system.class.value); }
			}
			if (item.type === "prayer") {
				...
				godList.forEach(god => {
					if (!this.gods.includes(god)) { this.gods.push(god); }
				});
			}
   ...
   */
	applyFilter (html) {
		...
		if (noItemFilter) { filteredItems = items; }
			...
			if (this.filters.dynamic[filter].show && this.filters.dynamic[filter].value) {
				switch (filter) {
					...
					case "qualitiesFlaws":
						if (this.filters.dynamic[filter].value.length && this.filters.dynamic[filter].value.some(x => x)) {
							filteredItems = filteredItems.filter(i => {
								if (!i.system.qualities.value.length && !i.system.flaws.value.length) { return false; }
								let properties = (Object.values(i.properties.qualities).concat(Object.values(i.properties.flaws))).map(i => i.display);
								if (!properties.length || (properties.length == 1 && properties[0] === "Special")) { return; }
					...
					case "symptoms": {
						if (this.filters.dynamic[filter].value.length && this.filters.dynamic[filter].value.some(x => x)) {
							filteredItems = filteredItems.filter(i => {
								if (!i.system.symptoms) { return true; }
					...
					case "skills":
					case "talents":
						if (this.filters.dynamic[filter].value.length && this.filters.dynamic[filter].value.some(x => x)) { filteredItems = filteredItems.filter(i => !i.system[filter] || (i.system[filter] && this.filters.dynamic[filter].value.every(value => { return i.system[filter].find(v => v.toLowerCase().includes(value.toLowerCase())); }))); }
						break;
					...
					case "protects":
						filteredItems = filteredItems.filter(i => {
							if (!i.system.AP) { return true; }
							let show;
							if (this.filters.dynamic.protects.value.head && i.system.AP.head) { show = true; }
							if (this.filters.dynamic.protects.value.body && i.system.AP.body) { show = true; }
							if (this.filters.dynamic.protects.value.arms && (i.system.AP.lArm || i.system.AP.rArm)) { show = true; }
							if (this.filters.dynamic.protects.value.legs && (i.system.AP.lLeg || i.system.AP.rLeg)) { show = true; }
							return show;
						});
						break;
					case "prayerType":
					...
					case "lore" :
						...
						break;
					default:
						if (this.filters.dynamic[filter].exactMatch) { filteredItems = filteredItems.filter(i => !i.system[filter] || (i.system[filter] && i.system[filter].value.toString().toLowerCase() == this.filters.dynamic[filter].value.toLowerCase())); } else { filteredItems = filteredItems.filter(i => !i.system[filter] || (i.system[filter] && i.system[filter].value.toString().toLowerCase().includes(this.filters.dynamic[filter].value.toLowerCase()))); }
						break;
				}
			}
		}
		...
		for (let element of list) {
			if (this.filterIds.includes(Number(element.getAttribute("data-filter-id")))) { $(element).show(); } else { $(element).hide(); }
		}
	...
	checkDynamicFilters (html) {
		for (let dynamicFilter in this.filters.dynamic) {
			...
			for (let typeFilter of this.filters.dynamic[dynamicFilter].type) {
				if (this.filters.type[typeFilter].value) { this.filters.dynamic[dynamicFilter].show = true; }
			}
	...
	activateListeners (html) {
		...
		html.on("change", ".boolean-filter", ev => {
			if ($(ev.currentTarget).hasClass("exactMatch")) { this.filters.dynamic[$(ev.currentTarget).attr("data-filter")].exactMatch = $(ev.currentTarget).is(":checked"); } else if ($(ev.currentTarget).attr("data-filter")) { this.filters.dynamic[$(ev.currentTarget).attr("data-filter")].value = $(ev.currentTarget).is(":checked"); }
