	...
	activateListeners (html) {
		...
		html.find(".issue-label").change(ev => {
			if (ev.currentTarget.value === "bug") {
				if (game.modules.contents.filter(i => i.active).map(i => i.id).filter(i => !this.domainKeys.includes(i)).length > 0) { modulesWarning.style.display = "block"; } else { modulesWarning.style.display = "none"; }
