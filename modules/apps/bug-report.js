...
export default class BugReportFormWfrp4e extends Application {
	...
	matchIssues (text) {
		if (this.issues instanceof Promise || !this.issues?.length) { return []; }
		...
		percentages.forEach((p, i) => {
			if (p > 0) { matchingIssues.push(this.issues[i]); }
		});
		return matchingIssues;
	}

	showMatchingGrudges (element, issues) {
		if (!issues || issues?.length <= 0) { element[0].style.display = "none"; } else {
	...
	activateListeners (html) {
		...
		html.find(".issue-label").change(ev => {
			if (ev.currentTarget.value == "bug") {
				if (game.modules.contents.filter(i => i.active).map(i => i.id).filter(i => !game.wfrp4e.config.premiumModules[i]).length > 0) { modulesWarning.style.display = "block"; } else { modulesWarning.style.display = "none"; }
			} else { modulesWarning.style.display = "none"; }
		});
		...
		html.find(".bug-submit").click(ev => {
			...
			if (!data.domain || !data.title || !data.description) { return ui.notifications.error(game.i18n.localize("BugReport.ErrorForm")); }
			if (!data.issuer) { return ui.notifications.error(game.i18n.localize("BugReport.ErrorName1")); }

			if (!data.issuer.includes("@") && !data.issuer.includes("#")) { return ui.notifications.notify(game.i18n.localize("BugReport.ErrorName2")); }
			...
			if (label) { data.labels.push(label); }
