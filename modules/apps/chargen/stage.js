...
export class ChargenStage extends FormApplication {
	...
	updateMessage (key, args = {}, string = null) {
		...
		if (this.options.message) {
			...
			if (string) { content += string; } else { content += game.i18n.format(`CHARGEN.Message.${key}`, args); }
	...
	async _onItemLookupClicked (ev) {
		...
		let item;
		if (name) { item = await WFRP_Utility.find(name, itemType); }

		if (item) {
			if (openMethod === "sheet") { item.sheet.render(true); } else { item.postItem(); }
		}
	}
}
