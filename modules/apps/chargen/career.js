...
export class CareerStage extends ChargenStage {
	...
	async _onDrop (ev) {
		...
		if (dragData.type === "Item") {
			...
			// If career level is not T1, find the T1 career and use that instead
				...
				if (careerT1[0]) { career = careerT1[0]; }
			}
	...
	async addCareerChoice (number = 1) {
		...
		if (this.data.subspecies && game.wfrp4e.tables.findTable("career", `${rollSpecies}-${this.data.subspecies}`)) { rollSpecies += `-${this.data.subspecies}`; }
   ...
   */
	async findT1Careers (careerNames) {
		if (typeof careerNames === "string") { careerNames = [careerNames]; }

		let packs = game.wfrp4e.tags.getPacksWithTag("career");
		let careers = game.items.filter(i => i.type === "career");
		let careersFound = [];

		for (let pack of packs) { careers = careers.concat((await pack.getDocuments()).filter(i => i.type === "career")); }

		// Find the tier 1 rank that corresponds with the career name
		for (let c of careers) {
			if (careerNames.includes(c.system.careergroup.value) && c.system.level.value == 1) { careersFound.push(c); }
			if (careersFound.length == careerNames.length) { break; }
		}

		if (careerNames.length != careersFound.length) { this.showError("CareerItems", {num: careerNames.length - careersFound.length, careers: careerNames.toString()}); }
		return careersFound;
	}
