...
export class AttributesStage extends ChargenStage {
	...
	async getData () {
		...
		if (this.context.step <= Step.FIRST_ROLL) {
			...
			this.context.exp = 25;
		} else { this.context.exp = 0; }

		return data;
	}

	async rollAttributes (ev, step) {
		if (step) { this.context.step = step; } else { this.context.step++; }
	...
	validateTotals () {
		...
		if (this.context.step == Step.ALLOCATING) {
			...
			for (let ch in this.context.characteristics) {
				let characteristic = this.context.characteristics[ch];
				if (characteristic.allocated < 4 || characteristic.allocated > 18) { inBounds = false; }
			}
	...
	swap (ch1, ch2) {
		if (this.context.step < Step.SWAPPING) { this.context.step = Step.SWAPPING; }
	...
	cancel (ev) {
		if (this.context.hasRerolled) { this.context.step = Step.REROLL; } else { this.context.step = Step.FIRST_ROLL; }
