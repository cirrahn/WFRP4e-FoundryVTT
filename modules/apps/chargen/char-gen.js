...
export default class CharGenWfrp4e extends FormApplication {
	...
	async _updateObject (ev, formData) {
		try {
			if (this.message) { this.message.update({content: this.message.content + game.i18n.format("CHARGEN.Message.Created", {name: this.data.details.name})}); }
			...
			for (let exp in this.data.exp) {
				if (Number.isNumeric(this.data.exp[exp])) { this.actor.system.details.experience.total += Number(this.data.exp[exp]); }
			}
	...
	activateListeners (html) {
		...
		html.find(".chargen-button").on("click", ev => {
			...
			if (stage.app) { stage.app.render(true); } else {
