...
export class SkillsTalentsStage extends ChargenStage {
	...
	constructor (...args) {
		...
		for (let talent of talents) {
			...
			} else { this.context.speciesTalents.normal.push(talent); }
		}
		...
		data.talents = {
			...
			choices: this.context.speciesTalents.choices.map((choice, index) => {
				return choice.split(",").map(i => {
					...
					if (match) {
						if (match[1] === "1") { name = game.i18n.localize("CHARGEN.AdditionalRandomTalent"); } else { name = game.i18n.format("CHARGEN.XAdditionalRandomTalents", { X: match[1] }); }
	...
	async _updateObject (ev, formData) {
		...
		for (let skill in this.context.speciesSkills) {
			if (isNaN(this.data.skillAdvances[skill])) { this.data.skillAdvances[skill] = 0; }
		...
		for (let talent in this.context.careerTalents) {
			if (this.context.careerTalents[talent]) { careerTalent = talent; }
		}
	...
	async validate () {
		...
		if (!this.validateSkills()) { valid = false; }
