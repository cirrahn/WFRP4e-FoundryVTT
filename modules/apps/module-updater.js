...
export default class ModuleUpdater extends Dialog {
		...
		super({
			...
			buttons:
            {
            	update:
              {
              	...
              	callback: html => {
              		if (!game.settings.get(module.id, "initialized")) { return ui.notifications.notify(game.i18n.localize("UPDATER.Error")); }
	...
	async updateImportedContent (settings) {
		...
		for (let type in settings) {
			if (type !== "excludeNameChange" && settings[type]) { await this.updateDocuments(documents[type], settings); }
		}
	...
	async updateDocuments (documents, settings) {
		...
		for (let document of documents) {
			...
			if (!documentClass) { documentClass = CONFIG[document.documentName].documentClass; }
			if (game[document.collectionName].has(document.id)) {
				...
				let newDoc = document.toObject();
				if (folder) { newDoc.folder = folder.id; }
				toCreate.push(newDoc);
