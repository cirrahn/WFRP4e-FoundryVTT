...
export default class CareerSelector extends FormApplication {
	...
	sortCareers () {
		...
		if (!this.careers.length) { return careerList; }

		this.careers.forEach((tier, i) => {
			try {
				let data = { link: tier.link, level: tier.level.value, img: tier.img, name: tier.name, index: i };
				let type = "outOfClass";
				if (this.currentCareer && this.currentCareer.class.value == tier.class.value) { type = "inClass"; }
				...
				} else { careerList[type][tier.careergroup.value] = [data]; }
			} catch (e) {
				ui.notifications.error(`Error when displaying ${tier.name}: ${e}`);
			}
		});

		for (let career in careerList.inClass) { careerList.inClass[career] = careerList.inClass[career].sort((a, b) => a.level > b.level ? 1 : -1); }
		for (let career in careerList.outOfClass) { careerList.outOfClass[career] = careerList.outOfClass[career].sort((a, b) => a.level > b.level ? 1 : -1); }
	...
	calculateMoveExp () {
		let exp = 0; let reasons = [];
		if (!this.selectedCareer) { return { exp }; }
