...
export default class StatBlockParser extends FormApplication {
	...
	static async parseStatBlock (statString, type = "npc") {
		...
		name = name.map(word => {
			if (word === "VON") { return word.toLowerCase(); }
		...
		let status = -1;
		if (blockArray[0].includes("(")) { status = blockArray[0]; } else if (blockArray[1].includes("(")) { status = blockArray[1]; }
		...
		for (let i = 0; i < characteristicNames.length; i++) {
			if (characteristicNames[i] === "Agi") { characteristicNames[i] = "Ag"; }
			if (characteristicNames[i].toLowerCase() === "m") {
				model.details.move.value = Number(characteristicValues[i]);
				continue;
			}
			if (characteristicNames[i].toLowerCase() === "w") { continue; }
		...
		while (true && skillBlockIndex >= 0) {
			skillBlockIndex++;
			if (skillBlockIndex == talentBlockIndexStart || skillBlockIndex == traitBlockIndexStart || skillBlockIndex == trappingBlockIndexStart || skillBlockIndex >= blockArray.length) { break; }

			skillBlock = skillBlock.concat(` ${blockArray[skillBlockIndex]}`);
		}
		while (true && talentBlockIndex >= 0) {
			talentBlockIndex++;
			if (talentBlockIndex == skillBlockIndexStart || talentBlockIndex == traitBlockIndexStart || talentBlockIndex == trappingBlockIndexStart || talentBlockIndex >= blockArray.length) { break; }

			talentBlock = talentBlock.concat(` ${blockArray[talentBlockIndex]}`);
		}
		while (true && traitBlockIndex >= 0) {
			traitBlockIndex++;
			if (traitBlockIndex == skillBlockIndexStart || traitBlockIndex == talentBlockIndexStart || traitBlockIndex == trappingBlockIndexStart || traitBlockIndex >= blockArray.length) { break; }

			traitBlock = traitBlock.concat(` ${blockArray[traitBlockIndex]}`);
		}
		while (true && trappingBlockIndex >= 0) {
			trappingBlockIndex++;
			if (trappingBlockIndex == skillBlockIndexStart || trappingBlockIndex == talentBlockIndexStart || trappingBlockIndex == traitBlockIndexStart || trappingBlockIndex >= blockArray.length) { break; }
		...
		for (let match of skillMatches) {
			...
			for (let search of skillSearches) {
				let skillItem;
				try { skillItem = await WFRP_Utility.findSkill(`${search.name} ${search.group ? `(${search.group})` : ""}`.trim()); } catch {}
				if (!skillItem) {
		...
		for (let match of talentMatches) {
			...
			let talentItem;
			try { talentItem = await WFRP_Utility.findTalent(talentName); } catch { }
			...
			if (talentName == game.i18n.localize("NAME.Doomed")) {
				...
				talentItem.name += ` (${talentSpec})`;
			} else if (talentSpec) { talentItem.name += ` (${talentSpec})`; }

			talentItem.system.advances.value = 1;

			if (Number.isNumeric(talentAdvances)) {
				for (let i = 1; i < talentAdvances; i++) { talents.push(talentItem); }
			}
