...
export default class MarketWfrp4e {
   ...
   */
	static creditCommand (amount, actor, options = {}) {
		...
		if (errorOccured) { moneyItemInventory = false; } else {
		...
		if (options.suppressMessage) { ui.notifications.notify(`${actor.name} received ${moneyToSend.gc}${game.i18n.localize("MARKET.Abbrev.GC")} ${moneyToSend.ss}${game.i18n.localize("MARKET.Abbrev.SS")} ${moneyToSend.bp}${game.i18n.localize("MARKET.Abbrev.BP")}`); } else { ChatMessage.create(WFRP_Utility.chatDataSetup(msg, "roll")); }
   ...
   */
	static payCommand (command, actor, options = {}) {
		...
		if (options.suppressMessage) { ui.notifications.notify(msg); } else { ChatMessage.create(WFRP_Utility.chatDataSetup(msg, "roll")); }
		return moneyItemInventory;
	}
