...
export default class NameGenWfrp {
	...
	static human = {
		forename (gender = "male") {
			...
			if (roll != 0) { option = nameGroup[roll].substr(1); }
	...
	static helf = {
         ...
         * 001
         */
				let useConnector = false; let useElement = false; let useEnd = false;
   ...
   */
	static generateName (options = { species: "human" }) {
		...
		if (options.species) { options.species = options.species.toLowerCase(); }
		if (options.gender) { options.gender = options.gender.toLowerCase(); }

		// If gender not recognize, remove it (roll male or female names randomly)
		if (!["male", "female"].includes(options.gender)) { delete options.gender; }
   ...
   */
	static evaluateNamePartial (namePartial) {
		let options = Array.from(namePartial.matchAll(/\((.+?)\)/g));
   ...
   */
	static evaluateChoices (choiceString) {
		if (!choiceString) { return choiceString; }
