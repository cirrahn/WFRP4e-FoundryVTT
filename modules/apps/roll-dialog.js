...
export default class RollDialog extends Dialog {
	...
	updateValues (html) {
		...
		if (!game.settings.get("wfrp4e", "mooAdvantage") && game.settings.get("wfrp4e", "autoFillAdvantage")) { modifier.value = Number(modifier.value) + (game.settings.get("wfrp4e", "advantageBonus") * this.advantage || 0) || 0; }
	...
	activateListeners (html) {
		...
		html.find(".dialog-bonuses").change(ev => {
			...
			$(ev.currentTarget).find("option").filter((o, option) => option.selected).each((o, option) => {
				if (option.dataset.modifier) { this.cumulativeBonuses.testModifier += Number(option.dataset.modifier); }
				if (option.dataset.successbonus) { this.cumulativeBonuses.successBonus += Number(option.dataset.successbonus); }
				if (option.dataset.slbonus) { this.cumulativeBonuses.slBonus += Number(option.dataset.slbonus); }
				if (option.dataset.difficultystep) { this.cumulativeBonuses.difficultyStep += Number(option.dataset.difficultystep); }
			});
			this.updateValues(html);
		});
		...
		this.userEntry.testModifier = Number(html.find("[name=\"testModifier\"]").change(ev => {
			...
			if (!game.settings.get("wfrp4e", "mooAdvantage") && game.settings.get("wfrp4e", "autoFillAdvantage")) { this.userEntry.testModifier -= (game.settings.get("wfrp4e", "advantageBonus") * this.advantage || 0) || 0; }
		...
		this.userEntry.successBonus = Number(html.find("[name=\"successBonus\"]").change(ev => {
			...
			if (game.settings.get("wfrp4e", "mooAdvantage")) { this.userEntry.successBonus -= (this.advantage || 0); }
			this.updateValues(html);
		}).val());
		...
		this.selectedHitLocation = html.find("[name=\"selectedHitLocation\"]").change(ev => {
			...
			if (ev.currentTarget.value && !["none", "roll"].includes(ev.currentTarget.value)) {
				...
				{ this.userEntry.calledShot = -20; } else { this.userEntry.calledShot = 0; }
			} else {
				this.userEntry.calledShot = 0;
			}
			this.updateValues(html);
		...
		if (!game.settings.get("wfrp4e", "mooAdvantage") && game.settings.get("wfrp4e", "autoFillAdvantage")) { this.userEntry.testModifier -= (game.settings.get("wfrp4e", "advantageBonus") * this.advantage || 0); } else if (game.settings.get("wfrp4e", "mooAdvantage")) { this.userEntry.successBonus -= (this.advantage || 0); }
	}
}
