	...
	async initialize () {
		return new Promise((resolve) => {
			fetch(`modules/${this.moduleKey}/initialization.json`).then(async r => r.json()).then(async json => {
				...
				for (let folder of createdFolders) { this.folders[folder.type][folder.name] = folder; }
	...
	async initializeDocuments () {
		...
		for (let pack of packList) {
			...
			for (let document of documents) {
				...
				if (folder) { document.updateSource({ "folder": this.folders[document.documentName][folder].id }); }
				if (document.getFlag(this.moduleKey, "sort")) { document.updateSource({ "sort": document.flags[this.moduleKey].sort }); }
			}
			try {
