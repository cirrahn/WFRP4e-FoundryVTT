   ...
   */
     ...
     */
		function extractAmountAndOptionFromCommandLine (commands) {
			let amount, optionInCommandLine;
 ...
 */
	...
	Hooks.on("renderChatMessage", async (app, html, msg) => {
		...
		if (!game.user.isGM) {
			...
			if (msg.message.speaker.actor && game.actors.get(msg.message.speaker.actor).ownership != 3) { html.find(".chat-button-player").remove(); }
		} else {
			...
			postedItem.addEventListener("dragstart", ev => {
				...
				if (game.user.isGM) {
					...
					renderTemplate("systems/wfrp4e/templates/chat/post-item.hbs", recreateData).then(html => {
						...
						if (newQuantity <= 0) { app.delete(); }
					});
				} else {
					let newQuantity = app.flags.postQuantity - 1;

					if (app.flags.postQuantity) { ev.dataTransfer.setData("text/plain", app.flags.transfer); }
