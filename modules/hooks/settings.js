		...
		if (setting.key === "wfrp4e.groupAdvantageValues") {
			...
			if (game.user.isGM && game.combat) {
				...
				game.combat.combatants.forEach(c => {
					if (c.actor.status.advantage.value != setting.value[c.actor.advantageGroup]) { c.actor.update({"system.status.advantage.value": setting.value[c.actor.advantageGroup]}, {fromGroupAdvantage: true}); }
				});
			}
