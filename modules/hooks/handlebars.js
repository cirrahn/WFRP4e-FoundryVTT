export default function () {
	Hooks.on("init", () => {
		...
		Handlebars.registerHelper("configLookup", function (obj, key) {
			if (obj && key) { return game.wfrp4e.config[obj]?.[key]; }
		});

		Handlebars.registerHelper("array", function (array, cls) {
			if (typeof cls === "string") { return array.map((value, index) => `<a data-index=${index} class="${cls}">${value}</a>`).join(`<h1 class="${cls} comma">, </h1>`); } else { return array.join(", "); }
		});
